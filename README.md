...
<p align="center">
  <a href="https://www.npmjs.com/package/@bbcli/pi-coding-agent"><img src="https://img.shields.io/npm/v/@bbcli/pi-coding-agent?style=flat&colorA=222222&colorB=CB3837" alt="npm version"></a>
...
`bun setup` installs Bun workspaces and builds `@bbcli/pi-natives`. Re-run `bun run build:native` after changing Rust crates or `packages/natives`.
...
| Package                                                                       | Description                                                                 |
| ----------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **[@bbcli/collab-web](packages/collab-web)**                               | Browser guest client, mock host, and local relay for collab live sessions   |
| **[@bbcli/pi-ai](packages/ai)**                                            | Multi-provider LLM client with streaming and model/provider integration     |
| **[@bbcli/pi-catalog](packages/catalog)**                                  | Model catalog: bundled model database, provider descriptors, and identity   |
| **[@bbcli/pi-agent-core](packages/agent)**                                 | Agent runtime with tool calling and state management                        |
| **[@bbcli/pi-coding-agent](packages/coding-agent)**                        | Interactive coding agent CLI and SDK                                        |
| **[@bbcli/pi-tui](packages/tui)**                                          | Terminal UI library with differential rendering                             |
| **[@bbcli/pi-natives](packages/natives)**                                  | N-API bindings for grep, shell, image, text, syntax highlighting, and more  |
| **[@bbcli/stats](packages/stats)**                                     | Local observability dashboard for AI usage statistics                       |
| **[@bbcli/omptype](packages/omptype)**                                     | ArkType-compatible schema validation with lazy JIT compilation              |
| **[@bbcli/pi-utils](packages/utils)**                                      | Shared utilities (logging, streams, dirs/env/process helpers)               |
| **[@bbcli/pi-wire](packages/wire)**                                        | Shared collab live-session protocol types and relay constants               |
| **[@bbcli/pi-mnemopi](packages/mnemopi)**                                  | Local SQLite memory engine for Oh My Pi agents                              |
| **[@bbcli/snapcompact](packages/snapcompact)**                             | Bitmap-frame context compression package and SQuAD eval suite               |
| **[@bbcli/browser-relay](packages/browser-relay)**                         | Chrome extension that lets the Eval browser API drive your existing tabs    |
| **[@bbcli/pi-metaharness](packages/metaharness)**                          | Unified benchmark runners, Harbor run storage, REST/SSE API, live dashboard |
| **[@bbcli/typescript-edit-benchmark](packages/typescript-edit-benchmark)** | Edit benchmark suite built on TypeScript source mutations                   |

### Rust Crates

| Crate                                              | Description                                                                                         |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **[pi-natives](crates/pi-natives)**                | Core Rust native addon (N-API `cdylib`) used by `@bbcli/pi-natives`; aggregates the crates below |
...
- [npm](https://www.npmjs.com/package/@bbcli/pi-coding-agent)
