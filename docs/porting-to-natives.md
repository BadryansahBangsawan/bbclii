...
This is the contributor path for moving a measured JS/TS hot path into `crates/pi-natives` and exposing it through `@bbcli/pi-natives`.
...
- lazy vcs wrapper: `native/vcs.js` / `vcs.d.ts` (`@bbcli/pi-natives/vcs`).
...
- Import the generated root symbol or intentional lazy subpath from `@bbcli/pi-natives`.
