...
```ts
import type { HookAPI } from "@bbcli/pi-coding-agent/extensibility/hooks";
...
`packages/coding-agent/src/extensibility/hooks/index.ts` and the package subpath `@bbcli/pi-coding-agent/extensibility/hooks` export:
...
The package root (`@bbcli/pi-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
