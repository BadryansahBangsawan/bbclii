...
- Key collaborators:
  - `packages/utils/src/ar` (`@bbcli/pi-utils/ar`) — unified archive registry: `parseArchivePathCandidates()` parses archive selectors, `readArchiveEntries()`/`writeArchive()` rewrite containers atomically.
