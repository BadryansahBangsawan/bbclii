...
- `packages/coding-agent/src/session/auth-storage.ts` — re-exports `AuthStorage` from `@bbcli/pi-ai`; API key + OAuth resolution order
...
1. Load built-in providers/models from `@bbcli/pi-catalog` (`getBundledProviders` / `getBundledModels`).
