...
- Package: `@bbcli/stats`; bin: `omp-stats`; main user path: `omp stats`.
...
- Package: public `@bbcli/omptype`; install with `bun add @bbcli/omptype`; requires Bun 1.3.14 or newer.
...
- Public surfaces: `@bbcli/omptype` for native authoring, `/typebox` and `/zod` for compatibility builders, and `/ark` for the alias-free ArkType compatibility facade.
...
- Package: private `@bbcli/typescript-edit-benchmark`; support library with no standalone bin.
...
- Package: private `@bbcli/pi-metaharness`; bin: `metaharness`.
...
- Package: private `@bbcli/browser-relay`; user command: `omp browser-relay`.
...
- Package: private `@bbcli/collab-web`; production client: <https://my.omp.sh/>.
...
- Package: public `@bbcli/snapcompact`; install with `bun add @bbcli/snapcompact`; requires
  Bun 1.3.14 or newer.
...
- Runtime constraint: rasterization and PNG encoding require `@bbcli/pi-natives`.
...
- Package: public `@bbcli/pi-mnemopi`; bin: `mnemopi`; requires Bun 1.3.14 or newer. Install globally with `bun add --global @bbcli/pi-mnemopi`, then run `mnemopi <command>`. From a source checkout, `bun packages/mnemopi/src/cli.ts <command>` runs the same entrypoint.
