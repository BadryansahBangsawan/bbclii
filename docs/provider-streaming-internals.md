...
This document explains how token/tool streaming is normalized in `@bbcli/pi-ai`, then propagated through `@bbcli/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@bbcli/pi-ai`
