...
`scripts/probe.ts`: `@bbcli/pi-ai` `completeSimple`; production-matching model/auth/provider behavior.
...
```ts
import { arkToWireSchema } from "@bbcli/pi-ai"; // or toolWireSchema(tool)
