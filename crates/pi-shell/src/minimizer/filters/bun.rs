...
mod tests {
	...
	fn bun_run_check_colon_compacts_workspace_success_noise() {
		...
		let out = filter(
			...
			"$ bun run check:tools && bun run --workspaces --if-present check\n$ biome check . \
			 ...
			 applied.\n@bbcli/pi-utils check: Checked 40 files in 11ms. No fixes \
			 applied.\n@bbcli/pi-utils check: $ tsgo -p tsconfig.json --noEmit\n@bbcli/pi-utils \
			 check: Exited with code 0\n@bbcli/pi-coding-agent check: Checked 1178 files in 287ms. \
			 No fixes applied.\n@bbcli/pi-coding-agent check: $ tsgo -p tsconfig.json \
			 --noEmit\n@bbcli/pi-coding-agent check: Exited with code 0\n",
			0,
		);

		assert!(out.text.contains("check:ts: passed"));
		assert!(out.text.contains("root biome: ok"));
		assert!(out.text.contains("@bbcli/pi-utils"));
		assert!(out.text.contains("@bbcli/pi-coding-agent"));
	...
	fn bun_run_check_timeout_preserves_ambiguous_success() {
		...
		let out = filter(
			&ctx,
			"@bbcli/pi-utils check: Checked 40 files in 11ms. No fixes applied.\n@bbcli/pi-utils \
	...
	fn bun_check_failure_keeps_diagnostic_and_emits_failed_status() {
		...
		let input = concat!(
			...
			"@bbcli/pi-utils check: $ tsgo -p tsconfig.json --noEmit\n",
			"@bbcli/pi-utils check: Exited with code 0\n",
			"@bbcli/pi-coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"src/tools/bash.ts(42,7): error TS2322: Type 'string' is not assignable to type \
			 'number'.\n",
			"@bbcli/pi-coding-agent check: Exited with code 1\n",
		);
	...
	fn bun_check_success_emits_passed_status_and_no_noise() {
		...
		let input = concat!(
			...
			"@bbcli/pi-utils check: $ tsgo -p tsconfig.json --noEmit\n",
			"@bbcli/pi-utils check: Exited with code 0\n",
			"@bbcli/pi-coding-agent check: Checked 1178 files in 287ms. No fixes applied.\n",
			"@bbcli/pi-coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"@bbcli/pi-coding-agent check: Exited with code 0\n",
		);
