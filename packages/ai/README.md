...
```bash
npm install @bbcli/pi-ai
```

## Quick Start

```typescript
import { getModel, stream, complete, Context, Tool, type } from "@bbcli/pi-ai";
...
```typescript
import { type Tool, type } from "@bbcli/pi-ai";
...
```typescript
import { stream, validateToolCall, Tool } from "@bbcli/pi-ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@bbcli/pi-ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@bbcli/pi-ai";
...
```typescript
import { getModel, stream } from "@bbcli/pi-ai";
...
```typescript
import { getProviders, getModels, getModel } from "@bbcli/pi-ai";
...
```typescript
import { Model, stream } from "@bbcli/pi-ai";
...
```typescript
import { getModel, complete, Context } from "@bbcli/pi-ai";
...
```typescript
import { Context, getModel, complete } from "@bbcli/pi-ai";
...
bundles. Bun's `fetch` does not consume the env var natively, so bbcli injects
...
```typescript
import { getEnvApiKey } from "@bbcli/pi-ai";
...
Authenticate via the [`bbcli`](https://omp.sh) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
bbcli auth-broker login              # interactive provider selection
bbcli auth-broker login anthropic    # login to a specific provider
bbcli auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
bbcli auth-broker list               # list supported providers
bbcli auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { getProviderDefinition } from "@bbcli/pi-ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@bbcli/pi-ai";
import * as fs from "node:fs";
