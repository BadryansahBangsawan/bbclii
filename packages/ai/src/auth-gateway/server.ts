/**
 * bbcli auth-gateway HTTP server.
 *
 ...
 * pure protocol translation: foreign wire → bbcli Context → pi-ai stream() →
 * bbcli events → foreign wire.
 *
 * Endpoints:
...
import { extractHttpStatusFromError, extractRetryHint, logger } from "@bbcli/pi-utils";
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (bbcli re-stamps every parsed Message). role +
 ...
 * (bbcli's own proxy discovery, Zed's openai_compatible provider, ...) read to
