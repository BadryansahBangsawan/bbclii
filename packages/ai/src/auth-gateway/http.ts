...
import { getInstallId } from "@bbcli/pi-utils";
...
const PASSTHROUGH_HEADER_NAMES: Record<string, true> = {
	...
	"openai-project": true,
	"openai-beta": true,
	// Codex / ChatGPT-OAuth backend headers (see @bbcli/pi-catalog/wire/codex).
 ...
 *
 * pi-native bbcli clients send `x-omp-install-id` / `x-omp-hostname` /
