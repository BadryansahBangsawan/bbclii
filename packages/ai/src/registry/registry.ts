import type { KnownProvider } from "@bbcli/pi-catalog";
import { authProviders } from "@bbcli/pi-catalog/compat/auth";
import type { AuthProviderId, LoginProviderId } from "@bbcli/pi-catalog/compat/auth-ids";
 ...
 * (`@bbcli/pi-catalog` `rules/auth/*.kdl`) in `/login` display order.
