...
// pre-existing `@bbcli/pi-ai/auth-storage` surface.
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		// Concurrent bbcli startups can race against WAL recovery and the schema
	 ...
	 * the connection: concurrent bbcli startups race WAL recovery and the leases
