 ...
 *
 * Dispatches a {@link streamSimple}-shaped request to an `bbcli auth-gateway`
 ...
 * containerized bbcli deployments (such as robomp slots) that
 * route every LLM call through a credential-holding sidecar so the slot
 * itself stays credential-free.
 */
import * as os from "node:os";
import { getAppName, getInstallId, readSseJson } from "@bbcli/pi-utils";
...
/**
 * Stream a turn through an `bbcli auth-gateway` over the pi-native protocol.
 *
