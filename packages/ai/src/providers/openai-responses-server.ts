/**
 * OpenAI Responses HTTP wire-format ↔ bbcli Context bridge for the auth-gateway.
 *
 * Inbound: parses `POST /v1/responses` request bodies into a {@link ParsedRequest}.
 * Outbound: encodes bbcli's {@link AssistantMessage} (and event stream) back into
...
import { logger, structuredCloneJSON } from "@bbcli/pi-utils";
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	// `store` is a stateful-storage hint that bbcli's gateway doesn't honour;
					...
					switch (ev.type) {
						...
						case "toolcall_end": {
							...
							if (cur.customWireName) {
								...
								// Standard JSON tool: arguments object on the bbcli side, the
