...
import type { Context, FetchImpl, Model, ModelSpec } from "@bbcli/pi-ai/types";
...
describe("resolveOpenAIRequestSetup User-Agent", () => {
	test("sets bbcli User-Agent on xAI when none is provided", () => {
...
describe("xAI stream User-Agent", () => {
	test("xAI Responses POST sends bbcli User-Agent", async () => {
		...
		expect(captured.userAgent).toMatch(/^bbcli\/\d+\.\d+\.\d+$/);
	});

	test("xAI OAuth Responses POST sends bbcli User-Agent", async () => {
		const captured = await captureStreamHeaders(
	...
	test("OpenAI Completions POST does not send bbcli User-Agent", async () => {
		const captured = await captureStreamHeaders(
