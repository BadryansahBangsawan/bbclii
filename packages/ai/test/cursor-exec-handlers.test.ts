...
import { streamCursor as lazyStreamCursor, setCursorProviderModule } from "@bbcli/pi-ai/providers/register-builtins";
import type { AssistantMessage, Context, CursorExecHandlers, Model, ToolResultMessage } from "@bbcli/pi-ai/types";
...
import type { McpResult, ReadResult } from "@bbcli/pi-catalog/discovery/cursor-proto";
import {
...
import { create, encodeJsonValue } from "@bbcli/pi-catalog/discovery/protobuf";
...
describe("Cursor system prompt encoding", () => {
	...
	it("maps ordered system prompts to global CursorRule entries", () => {
		...
		expect(rules[0]?.fullPath).toBe("/bbcli/system-prompt/0.mdc");
		expect(rules[1]?.fullPath).toBe("/bbcli/system-prompt/1.mdc");
