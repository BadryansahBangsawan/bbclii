...
import { getStatsDbPath, isEnoent, logger } from "@bbcli/pi-utils";
...
const TEMP_PATH_RE = /(?:^|\/)(?:T|tmp|pi-bash-exec|bbcli-bash-exec|pi-bash-detach)(?:\/|$)|^\/var\/folders(?:\/|$)/;
 ...
 *
 * Rules are generic: bbcli internal wt paths are dropped; conventional worktree
...
export function normalizeProjectPath(p: string): string | null {
	...
	if (/\/\.bbcli\/wt\//u.test(clean)) return null;
