...
/**
 * Native addon loader for `@bbcli/pi-natives`.
 *
...
function resolveLeafPackageDir(platformTag) {
	try {
		...
		return path.dirname(require_.resolve(`@bbcli/pi-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
...
export function validateLoadedBindings(ctx, bindings, candidate) {
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @bbcli/pi-natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @bbcli/pi-natives\n" +
