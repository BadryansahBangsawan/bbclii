...
// Re-exported from @bbcli/pi-utils so the whole workspace shares one
// `fetch`-compatible signature (tls-fetch's wrappers produce/accept it).
export type { FetchImpl } from "@bbcli/pi-utils";
...
export interface Model<TApi extends Api = Api> {
	 ...
	 * provider client. The `baseUrl` must point at an `bbcli auth-gateway`
	 * (or compatible) host; `headers.Authorization` (or `apiKey` resolved by
	 * the registry) carries the gateway bearer.
	 *
	 * Used by containerized bbcli installs (e.g. robomp slots) to route every
