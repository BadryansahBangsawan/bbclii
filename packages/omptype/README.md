...
```sh
npm install @bbcli/omptype
# or
bun add @bbcli/omptype
```
...
```ts
import { Type, type Static } from "@bbcli/omptype/typebox";
import { z } from "@bbcli/omptype/zod";
...
`@bbcli/omptype/ark` provides the repository's ArkType compatibility facade
