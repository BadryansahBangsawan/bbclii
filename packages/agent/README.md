...
Stateful agent with tool execution and event streaming. Built on `@bbcli/pi-ai`.

## Installation

```bash
npm install @bbcli/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@bbcli/pi-agent";
...
```typescript
import { Agent, streamProxy } from "@bbcli/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@bbcli/pi-agent";
