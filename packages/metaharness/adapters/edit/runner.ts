...
import { EditStore, hashlineCountOps, hashlineFormatHeader } from "@bbcli/pi-natives";
import type { AgentMessage, ResolvedThinkingLevel, ThinkingLevel } from "@bbcli/pi-agent-core";
import type { Model, ToolExample } from "@bbcli/pi-ai";
import { formatSessionDumpText, RpcClient } from "@bbcli/pi-coding-agent";
import { prompt } from "@bbcli/pi-utils";
import { diffLines } from "diff";
import { formatDirectory } from "@bbcli/typescript-edit-benchmark/formatter";
import {
	discoverSharedInfra,
	InProcessClient,
	type SharedInfra,
} from "@bbcli/typescript-edit-benchmark/in-process-client";
...
import type { EditTask } from "@bbcli/typescript-edit-benchmark/tasks";
import {
	verifyExpectedFileSubset,
	verifyExpectedFiles,
} from "@bbcli/typescript-edit-benchmark/verify";

const REPO_ROOT = path.resolve(import.meta.dir, "..", "..", "..", "..");
const RUNS_DIR = path.join(REPO_ROOT, "runs");
const TMP = path.join(RUNS_DIR, `rb-${Math.random().toString(36).slice(2, 10)}`);
const CLI_PATH = Bun.fileURLToPath(import.meta.resolve("@bbcli/pi-coding-agent/cli"));
