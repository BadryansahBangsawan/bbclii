 ...
 *
 * Auth: provider API keys resolve through bbcli's auth storage
 * (~/.bbcli/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import { parseArgs } from "node:util";
import { type Api, AuthStorage, completeSimple, type Model, SqliteAuthCredentialStore } from "@bbcli/pi-ai";
import { type GeneratedProvider, getBundledModel } from "@bbcli/pi-catalog/models";
import { getAgentDbPath } from "@bbcli/pi-utils";
...
async function openModel(modelSpec: string, storage: AuthStorage): Promise<OpenedModel> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (run \`bbcli login\` or set the provider env var)`);
	}
