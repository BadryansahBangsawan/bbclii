...
/** Build and cache self-contained bbcli binaries for the selected guest architectures. */
...
/** Install bbcli and gateway-only configuration into one running trial microVM. */
export async function installAgent(vm: TrialVm, binaries: AgentBinaries, gateway: GatewayConfig): Promise<string> {
	...
	if (!binary) throw new Error(`No bbcli binary available for guest architecture ${vm.arch}`);

	const entrypoint = "/opt/bbcli/bbcli";
	const mkdir = await vm.exec("mkdir -p /opt/bbcli");
	if (mkdir.exitCode !== 0) throw new Error(`Could not create /opt/bbcli: ${mkdir.stderr.trim()}`);
	await vm.copyTo(binary, entrypoint);
	const chmod = await vm.exec(`chmod 755 ${shellQuote(entrypoint)}`);
	if (chmod.exitCode !== 0) throw new Error(`Could not make bbcli executable: ${chmod.stderr.trim()}`);
	const warmup = await vm.exec(`${shellQuote(entrypoint)} --version`);
	if (warmup.exitCode !== 0) throw new Error(`bbcli --version failed: ${warmup.stderr.trim()}`);
