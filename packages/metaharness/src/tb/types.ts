 ...
 * bbcli runs inside that guest over raw RPC stdio carried by the Vibemon SDK's
 ...
 * - `agent.ts` — bbcli Linux binary build + guest install
...
/** Guest CPU architecture, normalized to bbcli binary naming. */
...
/** OpenRouter request-routing suffix applied by bbcli. */
...
export interface GatewayConfig {
	...
	url: string;
	/** Gateway bearer (`bbcli auth-gateway token`); `"no-auth"` when the gateway runs open. */
	token: string;
...
/** Host paths of self-contained bbcli Linux binaries, keyed by guest architecture. */
export interface AgentBinaries {
	x64?: string;
	arm64?: string;
	/** bbcli package version the binaries were built from (for run provenance). */
	version: string;
}
