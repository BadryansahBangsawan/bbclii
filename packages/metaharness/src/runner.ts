...
/**
 * Harbor benchmark runner for the local `bbcli` build.
 *
...
const SOURCE_SRC_MOUNT = "/opt/bbcli/src";
const SOURCE_BIN_MOUNT = "/opt/bbcli/bin";
...
export interface Config {
	...
	/** Extra args forwarded verbatim to the in-container bbcli CLI invocation (repeatable). */
	agentArgs: string[];
...
const HELP = `metaharness runner (local bbcli)
...
Model / agent:
  -m, --model <provider/model>   Model (repeatable). Default anthropic/claude-sonnet-4-6
      --agent <name>             bbcli (default) | oracle | nop | any harbor agent
      --install <source|local|published> bbcli install mode (default: source).
      ...
      --version <v>              bbcli version for published install (default: latest)
      --thinking <level>         off|minimal|low|medium|high|xhigh|max

      --tarball <path>           Reuse a prebuilt bbcli tarball (implies --install local, --no-build)
      --no-build                 Skip packing; reuse newest tarball in bench dir (--install local)
      --agent-arg <arg>          Extra arg forwarded verbatim to the in-container bbcli CLI (repeatable)
      --env <KEY[=VALUE]>        Forward env into bbcli container (repeatable).
...
Gateway (auth, no keys in container):
      ...
      --web-search               Enable bbcli web_search (off by default; can't auth via gateway)
 ...
 * `agent/bbcli.txt` JSONL. Only bytes appended since the previous call are read
...
function parseTrial(dir: string, name: string): Trial | null {
	...
	if (!fs.existsSync(resultPath)) {
		...
		// Realtime cost from the live agent bbcli.txt log, parsed incrementally.
		const probe = probeTrialCost(path.join(dir, "agent", "bbcli.txt"));
	...
	costProbes.delete(path.join(dir, "agent", "bbcli.txt"));
...
function writeReport(st: RenderState, benchDir: string, exitCode: number): string {
	...
	const successPct = tot.done > 0 ? (tot.pass / tot.done) * 100 : 0;
	const lines: string[] = [];
	const isOmp = st.cfg.agent === "bbcli";
	const argsLabel = agentArgsLabel(st.cfg);
...
function buildTarball(benchDir: string): string {
	process.stdout.write(dim("packing local bbcli (bun pm pack)…\n"));
...
/** Linux deps tree + mount plan for running bbcli straight from the mounted repo. */
...
function writeComposeOverlay(benchDir: string, cfg: Config, source: SourceMount | null): string | null {
	...
	const file = path.join(benchDir, "bbcli-compose-overlay.yaml");
...
/**
 * Env vars injected into the in-container bbcli run: every host `PI_*` knob (minus
...
async function runBenchmark(cfg: Config): Promise<BenchmarkRun> {
	...
	if (cfg.agent === "bbcli" && cfg.envType === "docker" && !which("docker")) {
	...
	if (cfg.agent === "bbcli" && cfg.install === "local" && !cfg.binaryArm64 && !cfg.binaryX64) {
		if (tarball) {
	...
	if (cfg.agent === "bbcli" && cfg.install === "source" && !cfg.binaryArm64 && !cfg.binaryX64) {
		source = prepareSourceDeps(cfg);
	}

	// models.yml (gateway)
	let modelsYaml = "";
	if (cfg.agent === "bbcli" && cfg.gateway) {
		modelsYaml = writeModelsYaml(benchDir, cfg);
		if (!gatewayHealthOk(cfg.gatewayUrl)) {
			process.stderr.write(
				yellow(
					`warning: gateway ${cfg.gatewayUrl} health check failed (continuing). Is the pm2 'bbcli-auth-gateway' running?\n`,
				),
			);
		}
	}
	...
	if (cfg.dryRun) {
		...
		process.stdout.write(bold("bbcli env:\n"));
