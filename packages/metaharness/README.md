...
1. **Local bbcli, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs bbcli
   ...
   `/opt/bbcli/bin` — so trial setup needs zero outbound network. Alternatives:
   `--install local` (pack a tarball per run) or `--binary` (prebuilt
   `dist/bbcli-linux-*` self-contained binaries).
...
image as an x86_64 KVM microVM on a Vibemon host, streams bbcli's `--mode rpc`
...
broker for its lifetime, and reverse-tunnels the loopback bbcli auth gateway so
...
| `--env <KEY[=VALUE]>` | — | Forward env into the bbcli container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt bbcli binary (repeat for arm64+x64) |
| `--install <source\|local\|published>` | `source` | `source` = repo bind-mount, `local` = tarball pack, `published` = npm `@bbcli/pi-coding-agent` |
...
arc; ~$0.001 per report). API keys resolve through bbcli's auth storage.
