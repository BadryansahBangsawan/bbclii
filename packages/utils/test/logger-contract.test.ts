...
async function runScenario(scenario: string): Promise<ScenarioResult> {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-logger-contract-"));
	roots.push(root);
...
async function logFileNames(directory: string): Promise<string[]> {
	return (await fs.readdir(directory))
		.filter(name => /^bbcli\.\d{4}-\d{2}-\d{2}\.\d+\.log(?:\.\d+)?$/.test(name))
		.sort();
}
...
describe("central logger byte contract", () => {
	test("pins levels, metadata normalization, key order, timestamp, errors, pid, and EOL", async () => {
		...
		expect(log.name).toBe(`bbcli.2026-01-01.${result.pid}.log`);
		const expected = [
		...
		expect(await fs.readFile(path.join(result.primaryDir, `.bbcli.${result.pid}-audit.json`), "utf8")).not.toBe("");
	});
...
describe("central logger transport lifecycle", () => {
	test("defaults to file-only without touching stdout or stderr", async () => {
		...
		expect(result.stdout).toBe("");
		expect(result.stderr).toBe("");
		const defaultLogsDir = path.join(result.primaryDir, ".bbcli", "logs");
...
describe("DailyRotateFile option and retention contract", () => {
	test("uses local-day names, a PID audit, SHA-256, and retains exactly five rotations", async () => {
		...
		expect(result.stdout).toBe("");
		expect(result.stderr).toBe("");
		const expectedNames = [2, 3, 4, 5, 6].map(day => `bbcli.2026-01-0${day}.${result.pid}.log`);
		...
		const auditPath = path.join(result.primaryDir, `.bbcli.${result.pid}-audit.json`);
	...
	test("crosses 10 MiB before rolling the following record to suffix .1", async () => {
		...
		expect(result.stdout).toBe("");
		expect(result.stderr).toBe("");
		const baseName = `bbcli.2026-01-01.${result.pid}.log`;
		...
		const audit = JSON.parse(
			await fs.readFile(path.join(result.primaryDir, `.bbcli.${result.pid}-audit.json`), "utf8"),
		) as AuditFile;
		expect(audit.keep).toEqual({ days: false, amount: 5 });
...
test("root and direct source entry points expose identical public logger functions", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-logger-api-"));
	roots.push(root);
