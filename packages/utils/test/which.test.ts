...
describe("$which", () => {
	...
	it.skipIf(process.platform === "win32")("uses the current process PATH for each cached lookup", () => {
		const firstDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-which-first-"));
		const secondDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-which-second-"));
		tempDirs.push(firstDir, secondDir);

		const command = `bbcli-which-${process.pid}`;
