...
describe("run() per-command help", () => {
	// Contract: `bbcli <cmd> --help` must load only the requested command module.
	...
	it("loads only the requested command", async () => {
		...
		try {
			await run({ bin: "bbcli", version: "0.0.0", argv: ["good", "--help"], commands });
		} finally {
...
describe("run() root help", () => {
	...
	it("renders static metadata without loading command modules", async () => {
		...
		try {
			await run({ bin: "bbcli", version: "0.0.0", argv: ["--help"], commands });
		} finally {
			stdoutSpy.mockRestore();
		}
...
describe("run() usage errors", () => {
	...
	it("prints a concise usage error instead of throwing on a missing required arg", async () => {
		...
		try {
			await expect(run({ bin: "bbcli", version: "0.0.0", argv: ["bench"], commands })).resolves.toBeUndefined();
		} finally {
		...
		expect(out).toContain("$ bbcli bench MODELS... [FLAGS]");
	...
	it("renders a required variadic arg without optional brackets", async () => {
		...
		try {
			await run({ bin: "bbcli", version: "0.0.0", argv: ["bench", "--help"], commands });
		} finally {
			stdoutSpy.mockRestore();
		}
	...
	it("prints a concise usage error for an unknown flag", async () => {
		...
		try {
			await expect(
				run({ bin: "bbcli", version: "0.0.0", argv: ["bench", "--unknown"], commands }),
