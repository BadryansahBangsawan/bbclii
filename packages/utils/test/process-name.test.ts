...
describe("setProcessName", () => {
	it.skipIf(os.platform() !== "linux")("sets /proc/self/comm on Linux", async () => {
		...
		// TASK_COMM_LEN caps comm at 15 chars; "bbcli-probe" fits whole.
		expect(report.comm).toBe("bbcli-probe");
		expect(report.title).toBe("bbcli-probe");
	});
});
