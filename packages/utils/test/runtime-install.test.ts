...
async function makeNodeModules(packages: Record<string, { manifest: Record<string, unknown>; files: string[] }>) {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-runtime-install-"));
	tempDirs.push(root);
...
describe("installRuntimeModuleResolver", () => {
	...
	test("uninstall restores the stock resolver and createRequire relative requires", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-runtime-uninstall-"));
		tempDirs.push(root);
...
describe("writeRuntimeManifest", () => {
	async function readManifest(install: Parameters<typeof writeRuntimeManifest>[1]) {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-runtime-manifest-"));
		tempDirs.push(dir);
...
describe("ensureRuntimeInstalled install lock", () => {
	...
	async function makeFileDependency(): Promise<{ spec: string; probe: string }> {
		const src = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-runtime-dep-"));
		tempDirs.push(src);
		await fs.writeFile(
			path.join(src, "package.json"),
			JSON.stringify({ name: "bbcli-runtime-fixture", version: "1.0.0" }),
		);
		return { spec: `file:${src}`, probe: "bbcli-runtime-fixture" };
	}

	async function makeRuntimeDir(): Promise<string> {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-runtime-cache-"));
		tempDirs.push(root);
