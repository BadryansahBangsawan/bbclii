 ...
 * crash restore). Unlike codex we redirect to the bbcli log file — not
...
export interface SuppressTerminalStderrOptions {
	/** Redirect target path; defaults to today's bbcli log file, then /dev/null. */
	redirectPath?: string;
...
export function suppressTerminalStderr(options?: SuppressTerminalStderrOptions): boolean {
	...
	try {
		...
		// on a fresh profile ~/.bbcli/logs may not exist yet. Create it here so
