/**
 * Set the OS-visible process name (`/proc/self/comm`) so `bbcli` shows up as
 * `bbcli` — not `bun` — in `ps`, `pgrep`, `killall`, `top`, `htop`, and systemd.
 *
 ...
 * `bun` and process-name-based tooling can't target bbcli (and `pkill bun` becomes
