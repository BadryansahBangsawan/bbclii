 ...
 * - **Dev runs only.** In the compiled `bbcli` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("bbcli.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("bbcli.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("bbcli.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("bbcli.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
