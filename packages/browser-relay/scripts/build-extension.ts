 ...
 * - `dist/bbcli-browser-relay-extension.zip` — packaged extension for GH releases
 * - `../coding-agent/src/tools/browser/relay/extension-assets/*.txt` —
 *   generated text assets embedded into the bbcli CLI so `bbcli browser-relay
...
const zip = await $`zip -qr ../bbcli-browser-relay-extension.zip .`.cwd(distExtension).nothrow();
if (zip.exitCode !== 0) {
...
console.log(`  ${path.join(dist, "bbcli-browser-relay-extension.zip")}`);
