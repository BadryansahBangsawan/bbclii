 ...
 * `BlobStore` keep writing to `~/.bbcli/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@bbcli/pi-coding-agent";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "bbcli:sessions:", // optional, this is the default
});
