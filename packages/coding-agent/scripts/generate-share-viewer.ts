#!/usr/bin/env bun
/**
 * Build the standalone share-viewer page the bbcli relay serves at `GET /s/<id>`.
 *
...
// Public artifacts use the bundled bbcli web themes rather than TUI themes.
const themeStyles = await generateThemeStyles("web");

const html = getTemplate()
	.replace("<theme-vars/>", () => `<style>${themeStyles}</style>`)
	.replace("<title>Session Export</title>", () => "<title>bbcli session</title>")
