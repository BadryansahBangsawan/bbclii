/**
 * Bun `--preload` shim for the bbcli dev launcher (`scripts/bbcli`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the bbcli CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@bbcli/pi-utils/dirs`).
 */
const launchCwd = process.env.BBCLI_LAUNCH_CWD ?? process.env.OMP_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.BBCLI_LAUNCH_CWD;
