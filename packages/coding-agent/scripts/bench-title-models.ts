 ...
 *   bun scripts/bench-title-models.ts --db ~/.bbcli/agent/history.db --out bench.json
 */
...
function parseArgs(argv: string[]): BenchConfig {
	...
	return {
		dbPath: (get("--db") ?? path.join(os.homedir(), ".bbcli/agent/history.db")).replace(/^~/, os.homedir()),
