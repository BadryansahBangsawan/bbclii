...
import { getActiveSkills } from "@bbcli/pi-coding-agent/extensibility/skills";
import type { Skill } from "@bbcli/pi-coding-agent/sdk";
...
function createExtensionSkill(packageDir: string, skillName: string): void {
	...
	fs.writeFileSync(
		...
		JSON.stringify({ name: path.basename(packageDir), bbcli: { extensions: ["./extension.ts"] } }),
	);
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in .bbcli/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".bbcli", "skills", "test-skill");
		fs.mkdirSync(skillsDir, { recursive: true });
		originalHome = process.env.HOME;
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-sdk-home-"));
		process.env.HOME = tempHomeDir;
		const nativeUserSkillsDir = path.join(tempHomeDir, ".bbcli", "agent", "skills");
...
`,
	...
	it("SDK invocation root scope isolates disabled discovery and merges normal discovery", async () => {
		...
		const installedPackage = path.join(tempHomeDir, ".bbcli", "plugins", "node_modules", "sdk-installed-extension");
		...
		fs.mkdirSync(path.join(tempHomeDir, ".bbcli", "plugins"), { recursive: true });
		fs.writeFileSync(
			path.join(tempHomeDir, ".bbcli", "plugins", "package.json"),
			JSON.stringify({ name: "bbcli-plugins", dependencies: { "sdk-installed-extension": "1.0.0" } }),
		);
		...
		const baseSessionOptions = {
			cwd: tempDir,
			agentDir: path.join(tempHomeDir, ".bbcli", "agent"),
			modelRegistry: sharedModelRegistry,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".bbcli", "agent");
	...
	it("refreshSkills reloads project skills on an existing session", async () => {
		...
		const runtimeSkillDir = path.join(tempDir, ".bbcli", "skills", "runtime-added-skill");
...
`,
	...
	it("manage_skill hot-registers managed skills in the active session", async () => {
		...
		const managedAgentDir = path.join(tempHomeDir, ".bbcli", "agent");
