 ...
 *
 * Strategy: spy on the six `@bbcli/pi-utils` plugin-path getters so the
...
describe("PluginManager.install with git sources", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-plugin-git-"));
		...
		vi.spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "bbcli-plugins.lock.json"));
	...
	test("installs from github: shorthand and resolves real package name from deps diff", async () => {
		...
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "bbcli-plugins", private: true, dependencies: {} }, null, 2),
		);
