...
import { ExtensionList } from "@bbcli/pi-coding-agent/modes/components/extensions/extension-list";
import {
	liveToolsForExtension,
	parseToolFileHeader,
	projectListHint,
	toolParamsFromSchema,
} from "@bbcli/pi-coding-agent/modes/components/extensions/inspector-model";
import { InspectorPanel } from "@bbcli/pi-coding-agent/modes/components/extensions/inspector-panel";
import type { Extension } from "@bbcli/pi-coding-agent/modes/components/extensions/types";
import { initTheme, theme } from "@bbcli/pi-coding-agent/modes/theme/theme";
import { shortenPath } from "@bbcli/pi-coding-agent/tools/render-utils";
...
function systemdExtension(): Extension {
	return {
		...
		path: "/home/sf/.bbcli/agent/tools/systemd.ts",
		source: userSource(),
		state: "active",
		raw: {
			name: "systemd",
...
function systemdFactory(sourcePath = "/home/sf/.bbcli/agent/tools/systemd.ts") {
	return [
		{
...
function toolExtension(): Extension {
	return {
		...
		path: "/home/sf/worlds/personal/.bbcli/tools/gmail_send.ts",
...
function commandExtension(): Extension {
	return {
		...
		displayName: "triage",
		trigger: "/triage",
		path: "/home/sf/worlds/_template/.bbcli/commands/triage.md",
		source: userSource(),
		state: "active",
		raw: {
			name: "triage",
...
function skillExtension(): Extension {
	return {
		...
		path: "/home/sf/.bbcli/agent/skills/hcom/SKILL.md",
		source: userSource(),
		state: "active",
		raw: {
			name: "hcom",
...
function listedSkillExtension(): Extension {
	return {
		...
		path: "/home/sf/.bbcli/agent/skills/fresh-drive/SKILL.md",
		source: userSource(),
		state: "active",
		raw: {
			name: "fresh-drive",
...
describe("tool inspector", () => {
	...
	test("collapses newlines in factory names and labels to one physical row", () => {
		...
		panel.setToolSource({
			...
			listLiveTools: () => [
				{
					name: "systemd_inspect\ninjected",
					sourcePath: "/home/sf/.bbcli/agent/tools/systemd.ts",
					label: "inspect\nlabel",
	...
	test("project list hints work for Windows .bbcli paths", () => {
		expect(
			projectListHint({
				...toolExtension(),
				path: "C:\\repo\\.bbcli\\tools\\x.ts",
			}),
		).toBe("repo");
	});

	test("project list hints work for Windows .bbcli paths with forward slashes", () => {
		expect(
			projectListHint({
				...toolExtension(),
				path: "C:/repo/.bbcli/tools/x.ts",
			}),
		).toBe("repo");
	});
	...
	test("joins a multi-export factory by originating source file", () => {
		...
		const list = new ExtensionList(
			[
				{
					...systemdExtension(),
					path: "/home/sf/worlds/personal/.bbcli/tools/systemd.ts",
					source: projectSource(),
				},
			],
			{ toolSource: systemdSource("/home/sf/worlds/personal/.bbcli/tools/systemd.ts") },
		);
		list.setFocused(true);
		const text = Bun.stripANSI(list.render(80).join("\n"));
	...
	test("does not join a shadowed custom-tool row against the winner's live schema", () => {
		...
		panel.setToolSource({
			getLiveTool: () => ({
				...
				description: "Send an email via gog for an authorized personal Gmail account.",
				source: "extension",
				sourcePath: "/home/sf/worlds/personal/.bbcli/tools/gmail_send.ts",
				parameters: { type: "object", properties: { to: { type: "string" } } },
			}),
		});
		panel.setExtension({
			...
			path: "/home/sf/.bbcli/agent/tools/gmail_send.ts",
			raw: {
				name: "gmail_send",
				description: "gmail_send custom tool",
				path: "/home/sf/.bbcli/agent/tools/gmail_send.ts",
...
describe("skill inspector", () => {
		...
		const list = new ExtensionList([
			{
				...
				path: "/home/sf/worlds/personal/.bbcli/skills/gog-google/SKILL.md",
				source: projectSource(),
			},
		]);
		list.setFocused(true);
	...
	test("project-only context files do not invent a project name without .bbcli", () => {
		const list = new ExtensionList([
			{
 ...
 *
 * Symlink: ~/.bbcli/agent/tools/cloak.ts → this file.
 */
