...
} from "@bbcli/pi-coding-agent/mcp/startup-events";
...
describe("mcp/startup-events — connection-status cross-module contract", () => {
	...
	it("sanitizes failure errors before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.bbcli/mcp.log`;
		...
		expect(message).not.toContain("\t");
		expect(message).toContain("broken: failed at   ~/.bbcli/mcp.log");
	});
	...
	it("sanitizes server names before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.bbcli`;
		const message = formatMCPConnectionStatusMessage({
		...
		expect(message).not.toContain("\n");
		expect(message).not.toContain("\t");
		expect(message).toContain("Connected: ~/.bbcli/connected   server.");
		expect(message).toContain("Failed: ~/.bbcli/broken server: missing command.");
		expect(message).toContain("Still connecting: ~/.bbcli/pending");
	});
