...
import { getProjectAgentDir, removeSyncWithRetries, Snowflake } from "@bbcli/pi-utils";
	...
	describe("project layer (on disk)", () => {
		...
		it("keeps failed project writes bound to their original cwd", async () => {
			...
			try {
				...
				expect(await Bun.file(path.join(bareProject, ".bbcli", "config.yml")).exists()).toBe(false);
			} finally {
				mkdirSpy.mockRestore();
			}

			await settings.flush();
			expect(YAML.parse(await Bun.file(path.join(startDir, ".bbcli", "config.yml")).text())).toEqual({
		...
		it("writes project model roles only to the project YAML", async () => {
			const settings = await Settings.init({ cwd: startDir, agentDir });
			const projectConfigPath = path.join(startDir, ".bbcli", "config.yml");
		...
		it("does not copy unedited roles from other project settings providers", async () => {
			await Bun.write(
				path.join(scopedProject, ".bbcli", "settings.json"),
			...
			expect(YAML.parse(await Bun.file(path.join(scopedProject, ".bbcli", "config.yml")).text())).toEqual({
		...
		it("reapplies only native model roles over normal project-provider precedence", async () => {
			...
			await Bun.write(
				path.join(scopedProject, ".bbcli", "config.yml"),
		...
		it("reports project roles over global role fallbacks", async () => {
			...
			await Bun.write(path.join(scopedProject, ".bbcli", "config.yml"), "modelRoles:\n  default: anthropic/project\n");
