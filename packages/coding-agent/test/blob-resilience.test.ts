...
function startUploadEdge(): UploadEdge {
	...
	const server = Bun.serve({
		...
		fetch: async request => {
			...
			if (url.pathname === "/.well-known/bbcli-blob-health") {
...
describe("public exposure health", () => {
	it("serves the private health route from the local blob origin", async () => {
		...
		expect((await fetch(`${baseUrl}/.well-known/bbcli-blob-health?nonce=test`)).status).toBe(204);
	});
