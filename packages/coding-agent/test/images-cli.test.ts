...
import type {
	...
	BlobBrokerStatus,
} from "@bbcli/pi-coding-agent/blob-broker/protocol";
import { ProviderFileCache } from "@bbcli/pi-coding-agent/blob-broker/provider-file-types";
import {
	type ImagesCliDependencies,
	type ImagesCommandArgs,
	type ImagesResolvedConfig,
	runImagesCommand,
} from "@bbcli/pi-coding-agent/cli/images-cli";
import { Settings } from "@bbcli/pi-coding-agent/config/settings";
import type { FetchImpl } from "@bbcli/pi-utils";
...
beforeEach(async () => {
	root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-images-cli-"));
