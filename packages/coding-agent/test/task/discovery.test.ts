...
async function writeOmpPluginAgent(home: string): Promise<void> {
	const userPluginsRoot = path.join(home, ".bbcli", "plugins");
	...
	await fs.writeFile(
		...
		JSON.stringify({ name: "loom", version: "1.0.0", bbcli: { version: "1.0.0" } }),
	);
	await fs.writeFile(
		path.join(userPluginsRoot, "package.json"),
		JSON.stringify({
			name: "bbcli-plugins-root",
			version: "0.0.0",
...
describe("discoverAgents", () => {
	...
	beforeEach(async () => {
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-task-agent-discovery-"));
		projectDir = path.join(tempHome, "project");
		await fs.mkdir(projectDir, { recursive: true });
	});

	afterEach(async () => {
		enableProvider("bbcli-plugins");
		clearOmpExtensionCliRoots();
	...
	test("loads OMP agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".bbcli", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".bbcli", "agents", "bbcli-test-agent.md"), OMP_AGENT_MD);
		...
		expect(names).toContain("bbcli-test-agent");
		expect(names).not.toContain("cc-test-agent");
		expect(projectAgentsDir).toBe(path.join(projectDir, ".bbcli", "agents"));
	});

	test("loads agents from OMP npm plugins under <home>/.bbcli/plugins/node_modules", async () => {
	...
	test("excludes OMP npm plugin agents when bbcli-plugins is disabled", async () => {
		await writeOmpPluginAgent(tempHome);
		disableProvider("bbcli-plugins");
	...
	test("CLI extension agents win over project `extensions:` settings on dedup", async () => {
		...
		// the skills/hooks/tools surface in discovery/bbcli-plugins.ts.
