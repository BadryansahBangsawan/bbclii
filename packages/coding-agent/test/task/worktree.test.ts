...
} from "@bbcli/pi-coding-agent/task/worktree";
import * as natives from "@bbcli/pi-natives";
import * as vcs from "@bbcli/pi-natives/vcs";
import { removeWithRetries, setWorktreesDir } from "@bbcli/pi-utils";
...
async function createGitRepo(): Promise<string> {
	const repo = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-worktree-"));
	tempDirs.push(repo);
...
describe("worktree isolation helpers", () => {
	...
	it("sizes an untracked symlink itself rather than its target", async () => {
		...
		const repo = await createGitRepo();
		const targetDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-worktree-symlink-target-"));
	...
	describe("git-backed worktree helpers", () => {
		...
		beforeAll(async () => {
			repo = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-worktree-"));
		...
		it("uses compact isolation paths that do not embed long task ids", async () => {
			...
			const worktreeBase = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-worktree-base-"));
		...
		describe("after rewinding the shared fixture", () => {
			...
			it("keeps the index clean when stash pop would conflict with a cherry-picked change", async () => {
				...
				expect(stashList).toContain("bbcli-task-merge");
			...
			it("commits isolated edits when parent dirt only changes nearby context", async () => {
				...
				const isoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-worktree-iso-"));
				...
				let branchName = `bbcli/task/${taskId}`;
				try {
...
describe("getRepoRoot", () => {
	...
	it("rejects pure jj workspaces with an actionable Jujutsu message", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-purejj-"));
		tempDirs.push(dir);
	...
	it("preserves the generic git-not-found error for directories without any repo", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-norepo-"));
		tempDirs.push(dir);
	...
	it("returns the nested git root when a git checkout lives under an outer jj workspace", async () => {
		...
		const outer = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-outerjj-"));
		tempDirs.push(outer);
...
describe("detachGitDir", () => {
	...
	async function makeLinkedWorktree(): Promise<{ main: string; wt: string; commonDir: string; baseSha: string }> {
		const main = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-detach-main-"));
		tempDirs.push(main);
	...
	async function copyTree(source: string): Promise<string> {
		const iso = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-detach-iso-"));
		tempDirs.push(iso);
	...
	it("severs a copied linked-worktree from the parent so task git ops stay isolated", async () => {
		...
		await runGit(wt, ["fetch", iso, "feature/a:refs/heads/bbcli-fetched"]);
		expect(await runGit(wt, ["rev-parse", "bbcli-fetched"])).toBe(taskCommit);
	});
	...
	it("leaves an already-independent full-copy checkout untouched", async () => {
		const src = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-detach-src-"));
		tempDirs.push(src);
	...
	it("carries filemode, split-index, and shallow state into the detached repo", async () => {
		...
		const origin = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-detach-origin-"));
	...
	it("detaches when sourceCommonDir is reached through a symlinked path", async () => {
		...
		const aliasBase = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-detach-alias-"));
	...
	it("keeps ensureIsolation from mutating a linked-worktree parent (rcopy backend)", async () => {
		...
		const worktreeBase = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-detach-wtbase-"));
		tempDirs.push(worktreeBase);
		const originalWorktreeDir = process.env.OMP_WORKTREE_DIR;
		delete process.env.OMP_WORKTREE_DIR;
		setWorktreesDir(worktreeBase);
...
describe("applyNestedPatches", () => {
	...
	beforeAll(async () => {
		fixtureParent = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-nested-fixture-"));
	...
	beforeEach(async () => {
		...
		parentRepo = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-nested-apply-"));
	...
	it("returns a stash-restore warning when pop conflicts with the agent commit", async () => {
		...
		expect(committedFiles.trim()).toBe("file.txt");
		expect(stashList).toContain("bbcli-isolation-");
	});
});
...
describe("commitToBranch preserves agent commits", () => {
	...
	beforeAll(async () => {
		fixtureRepo = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-commit-fixture-"));
	...
	beforeEach(async () => {
		...
		parent = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-commit-parent-"));
		isolation = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-commit-iso-"));
		await Promise.all([
	...
	it("preserves the agent's commit message after merge", async () => {
		...
		expect(result?.branchName).toBe(`bbcli/task/${taskId}`);
	...
	it("preserves every message when the agent makes multiple commits", async () => {
		...
		expect(result?.branchName).toBe("bbcli/task/multi");

		const merge = await mergeTaskBranches(parent, [
			{ branchName: result!.branchName!, taskId: "multi", baseSha: result!.baseSha! },
		]);
		expect(merge).toEqual({ failed: [], merged: ["bbcli/task/multi"] });
	...
	it("appends one trailing commit when the agent leaves uncommitted work after committing", async () => {
		...
		expect(result?.branchName).toBe("bbcli/task/leftover");
	...
	it("filters baseline WIP when the agent commits with git add -A", async () => {
		...
		expect(result?.branchName).toBe("bbcli/task/dirty-baseline");
		expect(aiMessage).not.toHaveBeenCalled();
		...
		expect(merge).toEqual({ failed: [], merged: ["bbcli/task/dirty-baseline"] });
	...
	it("falls back to the AI-generated message when the agent never committed", async () => {
		...
		expect(result?.branchName).toBe("bbcli/task/nocommit");
		expect(aiMessage).toHaveBeenCalledTimes(1);
	...
	describe("with baseline WIP overlapping the agent's changes (#4136)", () => {
		...
		it("commits an agent-only delta via --3way when WIP and agent modify unrelated hunks of the same tracked file", async () => {
			...
			expect(result?.branchName).toBe("bbcli/task/wip-tracked-file");
		...
		it("commits an untracked WIP file that the agent modifies inside isolation", async () => {
			...
			expect(result?.branchName).toBe("bbcli/task/wip-untracked");
		...
		it("commits a staged-new WIP file that the agent modifies inside isolation", async () => {
			...
			expect(result?.branchName).toBe("bbcli/task/wip-staged-new");
		...
		it("does not leak WIP-only files into the branch commit when the agent leaves them untouched", async () => {
			...
			expect(result?.branchName).toBe("bbcli/task/wip-filter");
		...
		it("still seeds WIP when the agent commits only baseline WIP and leaves the real edit uncommitted", async () => {
			...
			expect(result?.branchName).toBe("bbcli/task/wip-only-commit");
