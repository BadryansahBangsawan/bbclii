...
import { recoverOrphanedBackups } from "@bbcli/pi-coding-agent/session/session-listing";
import { SessionManager } from "@bbcli/pi-coding-agent/session/session-manager";
import { FileSessionStorage, MemorySessionStorage } from "@bbcli/pi-coding-agent/session/session-storage";
...
describe("SessionManager rewrite EPERM replacement fallback", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "bbcli-eperm-"));
	});
...
describe("FileSessionStorage.writeTextAtomic commitGuard cleanup", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "bbcli-guard-cleanup-"));
	});
