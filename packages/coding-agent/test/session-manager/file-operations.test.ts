...
import type { FileEntry, SessionHeader } from "@bbcli/pi-coding-agent/session/session-entries";
import { findMostRecentSession, resolveResumableSession } from "@bbcli/pi-coding-agent/session/session-listing";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-session-dir-test-"));
...
describe("SessionManager legacy session migration persistence", () => {
	...
	beforeEach(() => {
		...
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-session-manager-legacy-agent-"));
		setAgentDir(testAgentDir);
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-session-manager-legacy-"));
	});
