...
import { FileSessionStorage, type WriteTextAtomicOptions } from "@bbcli/pi-coding-agent/session/session-storage";
import type { SessionTitleUpdate } from "@bbcli/pi-coding-agent/session/session-title-slot";
...
describe("session title source persistence", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-title-source-"));
