...
import { cleanupEmptyMoveSession, SessionManager } from "@bbcli/pi-coding-agent/session/session-manager";
...
describe("move-session cleanup tracking", () => {
	...
	beforeEach(async () => {
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "bbcli-move-cleanup-"));
