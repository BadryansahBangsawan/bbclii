...
import {
	...
	toSkillFrontmatter,
	writeManagedSkill,
} from "@bbcli/pi-coding-agent/autolearn/managed-skills";
import { parseFrontmatter, removeWithRetries } from "@bbcli/pi-utils";
...
describe("managed-skills primitives", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-managed-skills-"));
	...
	describe("writeManagedSkill", () => {
		...
		it("refuses a traversal name without writing outside the managed dir", async () => {
			...
			const authoredEvil = path.join(tempHome, ".bbcli", "agent", "skills", "evil", "SKILL.md");
		...
		it("refuses to write through a symlinked skill directory", async () => {
			...
			const outside = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-escape-"));
			try {
		...
		it("refuses to write when the managed-skills root itself is a symlink", async () => {
			const realRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-realroot-"));
			try {
		...
		it("refuses to update a SKILL.md that is a symlink", async () => {
			...
			const outside = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-link-"));
	...
	describe("deleteManagedSkill", () => {
		...
		it("refuses to delete through a symlinked skill directory", async () => {
			const managedRoot = getManagedSkillsDir();
			await fs.mkdir(managedRoot, { recursive: true });
			const outside = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-deltarget-"));
