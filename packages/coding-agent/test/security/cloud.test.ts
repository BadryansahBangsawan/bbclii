...
describe("Codex Security cloud client", () => {
	...
	test("imports cloud findings into the canonical store and SARIF", async () => {
		const repositoryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cloud-security-repo-"));
		const stateRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cloud-security-state-"));
	...
	test("refuses cloud imports when repository identity cannot be verified", async () => {
		const repositoryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cloud-security-no-origin-repo-"));
		const stateRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cloud-security-no-origin-state-"));
		const store = await SecurityStore.open(repositoryRoot, { stateRoot });
		const client = new CodexSecurityCloudClient({
			authStorage: authStorage(),
			account: ACCOUNT,
	...
	test("refuses to import a cloud configuration for another repository", async () => {
		const repositoryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cloud-security-mismatch-repo-"));
		const stateRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cloud-security-mismatch-state-"));
		await $`git init --initial-branch=main`.cwd(repositoryRoot).quiet();
