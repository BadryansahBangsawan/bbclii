...
describe("runPluginCommand({ action: 'install', args: [<local>] })", () => {
	...
	beforeEach(async () => {
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "bbcli-plugins.lock.json"));
	...
	test("real local plugin directory: install symlinks it like link would", async () => {
		...
		const lock = await Bun.file(path.join(tmpRoot, "bbcli-plugins.lock.json")).json();
