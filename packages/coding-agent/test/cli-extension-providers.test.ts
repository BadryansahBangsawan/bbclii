 ...
 *
 * One-shot CLIs (`bbcli bench`, dry-balance) build a bare `ModelRegistry` that
 ...
 * `~/.bbcli/agent/extensions/`) was invisible to model resolution and
 * `bbcli bench <provider>/<model>` failed with "Model not found".
 *
 ...
 * passed explicitly so the test never touches the developer's real `~/.bbcli`.
 */
...
import { getModelMatchPreferences, resolveCliModel } from "@bbcli/pi-coding-agent/config/model-resolver";
import { resetSettingsForTest, Settings } from "@bbcli/pi-coding-agent/config/settings";
import { loadCliExtensionProviders } from "@bbcli/pi-coding-agent/sdk";
