...
import { loadHooks } from "@bbcli/pi-coding-agent/extensibility/hooks/loader";
import { ExtensionExitError, withHostGuard } from "@bbcli/pi-coding-agent/extensibility/utils";
...
describe("extension/hook loader process.exit guard (#3680)", () => {
	...
	beforeEach(() => {
		project = TempDir.createSync("@bbcli-exit-guard-");
	});
	...
	const runGuardedShutdownProbe = (trigger: "sigint" | "fatal") => {
		...
		return runProbe(`
import { postmortem } from "@bbcli/pi-utils";
import { withHostGuard } from "@bbcli/pi-coding-agent/extensibility/utils";
