...
import { applyResolvedSystemPromptInputs, readPipedInput, submitInteractiveInput } from "@bbcli/pi-coding-agent/main";
import type { SubmittedUserInput } from "@bbcli/pi-coding-agent/modes/types";
import type { CreateAgentSessionOptions } from "@bbcli/pi-coding-agent/sdk";
import { discoverTitleSystemPromptFile } from "@bbcli/pi-coding-agent/system-prompt";
...
describe("discoverTitleSystemPromptFile", () => {
	it("discovers TITLE_SYSTEM.md from the project bbcli config directory", async () => {
		const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-title-system-"));
