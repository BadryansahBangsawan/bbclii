...
import { getConfigRootDir, getLogsDir, removeWithRetries, setAgentDir } from "@bbcli/pi-utils";
...
describe("report bundle logs", () => {
	it("collects every same-day PID log, not only the current process", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-report-logs-"));
		...
		const crashedName = `bbcli.${today}.4242.log`;
		const rotatedName = `${crashedName}.1`;
		const currentName = `bbcli.${today}.${process.pid}.log`;
