...
import { RawSseDebugBuffer } from "@bbcli/pi-coding-agent/debug/raw-sse-buffer";
import { createReportBundle } from "@bbcli/pi-coding-agent/debug/report-bundle";
...
describe("raw SSE report bundle", () => {
	it("includes captured raw SSE text and dropped-record disclosure", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-raw-sse-report-"));
		const xdgStateHome = path.join(cleanupRoot, "state");
		await fs.mkdir(path.join(xdgStateHome, "bbcli"), { recursive: true });
		...
		expect(rawSseText).toContain(": bbcli-debug-dropped records=");
