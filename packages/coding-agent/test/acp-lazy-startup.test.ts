...
import { createAcpConnection } from "@bbcli/pi-coding-agent/modes/acp/acp-mode";
...
beforeAll(() => {
	startupDir = TempDir.createSync("@bbcli-acp-startup-shared-");
...
describe("ACP lazy startup", () => {
	it("applies schema defaults for ACP background jobs", async () => {
		const { runRootCommand } = await import("@bbcli/pi-coding-agent/main");
	...
	it("applies CLI runtime API keys after ACP lazy session creation resolves extension models", async () => {
		using tempDir = TempDir.createSync("@bbcli-acp-lazy-api-key-");
...
`,
		...
		try {
			...
			const { runRootCommand } = await import("@bbcli/pi-coding-agent/main");
			const { createAgentSession } = await import("@bbcli/pi-coding-agent/sdk");
