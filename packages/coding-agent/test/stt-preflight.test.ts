...
import { Settings, settings } from "@bbcli/pi-coding-agent/config/settings";
import * as asrClient from "@bbcli/pi-coding-agent/stt/asr-client";
import * as downloader from "@bbcli/pi-coding-agent/stt/downloader";
import { STTController } from "@bbcli/pi-coding-agent/stt/stt-controller";
import { getTinyModelsCacheDir, removeWithRetries, setAgentDir } from "@bbcli/pi-utils";
...
describe("isSttModelCached completeness", () => {
	...
	beforeEach(async () => {
		...
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-stt-cache-"));
		setAgentDir(tmp);
