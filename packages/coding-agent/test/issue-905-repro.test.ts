 ...
 *
 * Contract under test: the `bbcli models` listing entry point loads extensions
...
`,
	...
	await fs.writeFile(
		...
		JSON.stringify({ name: "explicit-package", bbcli: { extensions: ["./src/main.ts"] } }),
	);
	await fs.writeFile(
...
test("bbcli models surfaces extension-registered providers (issue #905)", async () => {
...
test("bbcli models does not execute ambient hooks while retaining explicit providers", async () => {
...
test("bbcli models emits extension shutdown after listing (issue #6297)", async () => {
	const authStorage = await AuthStorage.create(":memory:");
	try {
		const modelRegistry = new ModelRegistry(authStorage);
		await runModelsListing({
...
test("bbcli models explicit-only mode resolves a package and excludes settings providers", async () => {
	const authStorage = await AuthStorage.create(":memory:");
	try {
		const modelRegistry = new ModelRegistry(authStorage);
		const captured: string[] = [];
...
test("bbcli models prints invalid models.yml schema errors before listing output", async () => {
