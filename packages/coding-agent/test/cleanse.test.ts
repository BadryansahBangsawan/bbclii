...
import * as cleanseAgent from "@bbcli/pi-coding-agent/cleanse/agent";
import * as cleanseCheckers from "@bbcli/pi-coding-agent/cleanse/checkers";
import { runCleanseCommand } from "@bbcli/pi-coding-agent/cleanse/index";
import { runCleanseLoop } from "@bbcli/pi-coding-agent/cleanse/loop";
import { type CleanseParserKind, parseCleanseDiagnostics } from "@bbcli/pi-coding-agent/cleanse/parsers";
import type {
	CleanseAgentOutcome,
	CleanseAssignment,
	CleanseDiagnostic,
	CleanseDiagnosticReport,
} from "@bbcli/pi-coding-agent/cleanse/types";
import { createProgressReporter } from "@bbcli/pi-coding-agent/cli/progress-reporter";
import { resolveCliArgv } from "@bbcli/pi-coding-agent/cli-commands";
...
describe("cleanse diagnostics", () => {
	...
	test("discovers package test scripts only when test mode is enabled", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cleanse-tests-"));
		try {
			await Bun.write(
	...
	test("excludes generated Bazel trees from checker discovery", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cleanse-ignore-"));
		try {
			await Bun.write(path.join(root, "src", "index.ts"), "export const value = 1;\n");
			await Bun.write(
...
describe("cleanse custom suite", () => {
	test("builds runnable plans from discovery specs and skips unrunnable ones", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cleanse-custom-"));
		try {
	...
	test("select() narrows which checkers a run executes", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cleanse-select-"));
		try {
			const suite = await cleanseCheckers.buildCustomCleanseSuite(root, [
	...
	test("streams partial diagnostics before a long-running checker exits", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cleanse-stream-"));
		try {
