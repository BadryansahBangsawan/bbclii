...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through bbcli", async () => {
		...
		expect(result.command).toBe("bbcli --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("bbcli-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command bbcli --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed bbcli binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("bbcli-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("bbcli-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/bbcli-profiles.fish") ?? "";
		expect(content).toContain("function bbcli-work --wraps bbcli");
		expect(content).toContain("command bbcli --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/bbcli-profiles.fish");
		expect(files.get(result.configPath)).toContain("function bbcli-work --wraps bbcli");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function bbcli-work");
		expect(content).toContain("& bbcli --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& bbcli --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> bbcli profile alias: bbcli-work >>>",
					"alias bbcli-work='command bbcli --profile=old'",
					"# <<< bbcli profile alias: bbcli-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command bbcli --profile=work "$@"');
	...
	it("refuses to shadow the base bbcli command case-insensitively", async () => {
		for (const aliasName of ["bbcli", "OMP"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/bbcli-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/bbcli-profiles.fish'");
	});
