...
import type { AssistantMessage, Context, Message, TextContent } from "@bbcli/pi-ai";
import {
	...
	loadSecrets,
} from "@bbcli/pi-coding-agent/secrets";
import {
...
} from "@bbcli/pi-coding-agent/secrets/message-transform";
import { type SecretEntry, SecretObfuscator } from "@bbcli/pi-coding-agent/secrets/obfuscator";
import {
...
} from "@bbcli/pi-coding-agent/secrets/placeholder";
import { compileSecretRegex } from "@bbcli/pi-coding-agent/secrets/regex";
import { getActiveProfile, getAgentDir, setProfile } from "@bbcli/pi-utils/dirs";
...
describe("lazy placeholder key", () => {
	...
	it("getSecretPlaceholderKeySync creates the key file on demand and shares it with the async readers", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-lazy-placeholder-key-"));
		try {
...
describe("getSecretPlaceholderKey", () => {
	...
	async function withTempAgentHome(run: () => Promise<void>): Promise<void> {
		...
		const tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-secret-key-"));
...
describe("SecretObfuscator friendlyName placeholders", () => {
	...
	it("omits invalid friendlyName metadata without dropping the secret", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-secret-friendly-"));
		try {
			const project = path.join(root, "project");
			const agentDir = path.join(root, "agent");
			await fs.mkdir(path.join(project, ".bbcli"), { recursive: true });
			await fs.mkdir(agentDir, { recursive: true });
			await fs.writeFile(
				path.join(project, ".bbcli", "secrets.yml"),
