 ...
 *
 * Note: helpers.ts imports @bbcli/pi-natives (Rust addon via glob).
...
} from "@bbcli/pi-coding-agent/discovery/helpers";
import type { InstalledPluginEntry } from "@bbcli/pi-coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@bbcli/pi-coding-agent/extensibility/plugins/marketplace";
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-proj-scope-"));
	});
	...
	it("walk-up finds nearest .bbcli/ directory", async () => {
		// Layout: tmpDir/.bbcli/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .bbcli/ there.
		fs.mkdirSync(path.join(tmpDir, ".bbcli"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".bbcli", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .bbcli/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.bbcli/   +   tmpDir/sub/.bbcli/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.bbcli/, not climb further to tmpDir/.bbcli/.
		fs.mkdirSync(path.join(tmpDir, ".bbcli"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".bbcli"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".bbcli", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .bbcli/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .bbcli/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .bbcli/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .bbcli/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.bbcli/.../installed_plugins.json.
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-proj-scope-home-"));
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".bbcli", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-shadow-proj-"));

		// Create .bbcli/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".bbcli", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".bbcli", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".bbcli", "plugins", "installed_plugins.json");
	});
