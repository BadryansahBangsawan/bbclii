 ...
 * @bbcli/pi-natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. OMP registry lives at path.join(home, ".bbcli", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".bbcli")
...
// Kept here to avoid importing helpers.ts (which pulls in @bbcli/pi-natives).
...
// Matches getConfigDirName() — single source of truth is in @bbcli/pi-utils,
// but we know the value is ".bbcli" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".bbcli";
...
let tmpHome: string;
/** ~/.bbcli/plugins/installed_plugins.json inside tmpHome */
let ompRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-discovery-test-"));
...
describe("OMP registry path contract", () => {
	it("OMP registry lives at home/.bbcli/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".bbcli", "plugins", "installed_plugins.json");
...
describe("OMP precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — OMP entry has required fields for deduplication", () => {
		...
		const ompEntry = makeEntry("/bbcli/cached/path");
