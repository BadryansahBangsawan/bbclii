...
test("getEnabledPlugins caches repeated discovery for the same cwd and home until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-plugin-cache-"));
	tempRoots.push(root);
	const home = path.join(root, "home");
	const cwd = path.join(root, "project");
	const pluginsDir = path.join(home, ".bbcli", "plugins");
	const pluginPackageJson = path.join(pluginsDir, "node_modules", "bbcli-cache-repro", "package.json");
	await fs.mkdir(path.dirname(pluginPackageJson), { recursive: true });
	await fs.mkdir(cwd, { recursive: true });
	await writeJson(path.join(pluginsDir, "package.json"), { dependencies: { "bbcli-cache-repro": "1.0.0" } });
	await writeJson(path.join(pluginsDir, "bbcli-plugins.lock.json"), {
		plugins: { "bbcli-cache-repro": { version: "1.0.0", enabled: true, enabledFeatures: null } },
		settings: {},
	});
	await writeJson(pluginPackageJson, {
		name: "bbcli-cache-repro",
		version: "1.0.0",
		bbcli: { tools: "tools" },
	});
...
test("legacy bare dependency rewrites cache fallback package resolution until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-legacy-cache-"));
	tempRoots.push(root);
