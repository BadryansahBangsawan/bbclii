...
import * as configModule from "@bbcli/pi-coding-agent/config";
import * as shim from "@bbcli/pi-coding-agent/extensibility/legacy-pi-coding-agent-shim";
import * as utils from "@bbcli/pi-utils";
...
describe("legacy shim path helpers", () => {
	...
	it("getPackageDir resolves the coding-agent package root in source mode", () => {
		// bbcli's canonical helper returns the package root containing package.json
	...
	// `path.join(getPackageDir(), ...)`. bbcli's canonical helper returns
