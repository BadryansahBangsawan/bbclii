...
describe("legacy pi compat package-root override validation (issue #2168)", () => {
	it("keeps overrides whose filesystem targets exist", () => {
		const candidates = {
			"@bbcli/pi-ai": "/tmp/exists-ai.js",
			"@bbcli/pi-utils": "/tmp/exists-utils.js",
		};
	...
	it("drops overrides whose filesystem targets are missing on disk", () => {
		const candidates = {
			"@bbcli/pi-ai": "/tmp/exists-ai.js",
			"@bbcli/pi-coding-agent": "/tmp/exists-shim.js",
			"@bbcli/pi-utils": "/$bunfs/root/packages/utils/src/index.js",
			"@bbcli/pi-tui": "/$bunfs/root/packages/tui/src/index.js",
		};
		...
		expect(result).not.toHaveProperty("@bbcli/pi-utils");
		expect(result).not.toHaveProperty("@bbcli/pi-tui");
	});
