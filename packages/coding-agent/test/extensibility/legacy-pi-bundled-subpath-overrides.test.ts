...
import { __buildLegacyPiPackageRootOverrides } from "@bbcli/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
`,
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		expect(bundledModuleKeys.has("@bbcli/pi-ai/oauth/openai-codex")).toBe(true);
	});
	...
	it("actually loads the shim's shared Pi translation through the bundled registry", async () => {
		...
		const key = "@bbcli/pi-ai/providers/cursor-pi-args";
...
`,
	...
	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@bbcli/pi-coding-agent/web/search/providers/xai",
			"@bbcli/pi-coding-agent/web/search/providers/tinyfish",
			"@bbcli/pi-coding-agent/web/search/providers/firecrawl",
			"@bbcli/pi-coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@bbcli/pi-coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@bbcli/pi-coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@bbcli/pi-coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@bbcli/pi-coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("does not register subpath overrides in dev/install mode", () => {
		...
		expect(overrides).not.toHaveProperty("@bbcli/pi-ai/oauth");
		expect(overrides).not.toHaveProperty("@bbcli/pi-coding-agent/tools");
	...
	it("bundles nested wildcard subpaths so a compiled extension can import them", () => {
		...
		expect(bundledModuleKeys.has("@bbcli/pi-coding-agent/slash-commands/helpers/active-oauth-account")).toBe(true);
		// Directory index modules stay excluded: `./x/*` must not serve `x/y`
		// from `y/index.ts`, which Node would not resolve either.
		expect(bundledModuleKeys.has("@bbcli/pi-coding-agent/modes/theme/defaults/index")).toBe(false);
	});
});
