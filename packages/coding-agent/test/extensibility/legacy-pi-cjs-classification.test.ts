...
import { loadLegacyPiModule } from "@bbcli/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
async function writePackage(files: Record<string, string>): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-cjs-classification-"));
	tempRoots.push(dir);
