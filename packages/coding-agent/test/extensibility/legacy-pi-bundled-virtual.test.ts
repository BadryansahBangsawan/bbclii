...
// `@bbcli/pi-*` imports through virtual modules backed by live host module
...
describe("legacy-pi bundled virtual module synthesizer (issue #3423)", () => {
	...
	it("emits one ES named export per enumerable namespace key", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@bbcli/pi-coding-agent", modules);
		expect(src).toContain(
			`const __omp_bundled = globalThis[${JSON.stringify(globalKey)}]["@bbcli/pi-coding-agent"];`,
		);
	...
	it("forwards `default` through `export default` so default imports survive", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@bbcli/pi-utils", modules);
	...
	it("throws when asked to synthesize a key the bundled modules do not cover", () => {
		expect(() => __synthesizeLegacyPiBundledSourceWithModules("@bbcli/pi-not-bundled", modules)).toThrow(
	...
	it("routes Bun plugin resolution through the bundled namespace so onLoad can serve extension imports", async () => {
		...
		expect(onLoadPaths).toEqual(["@bbcli/pi-utils"]);
		...
		expect(bundledModule.legacyAnswer).toBe("served:@bbcli/pi-utils");
	});
});
