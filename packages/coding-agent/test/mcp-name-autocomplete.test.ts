...
import { collectMcpServerNames } from "@bbcli/pi-coding-agent/modes/controllers/mcp-command-controller";
import type { InteractiveModeContext } from "@bbcli/pi-coding-agent/modes/types";
import { buildTuiBuiltinSlashCommands } from "@bbcli/pi-coding-agent/slash-commands/builtin-registry";
...
describe("MCP server-name autocomplete", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-autocomplete-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-autocomplete-agent-"));
