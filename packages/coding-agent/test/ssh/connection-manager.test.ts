...
import * as connectionManager from "@bbcli/pi-coding-agent/ssh/connection-manager";
import { removeWithRetries } from "@bbcli/pi-utils";

async function withLooseKey<T>(run: (keyPath: string) => Promise<T>): Promise<T> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-ssh-key-"));
...
describe("buildRemoteCommand", () => {
	...
	it("still rejects directory identity paths on Windows args", async () => {
		const keyPath = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-ssh-key-"));
		try {
			await expect(
				connectionManager.buildRemoteCommand(
					{
