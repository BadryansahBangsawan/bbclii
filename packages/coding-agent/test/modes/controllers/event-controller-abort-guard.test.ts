...
import { SETTINGS_SCHEMA } from "@bbcli/pi-coding-agent/config/settings-schema";
...
import * as titleGenerator from "@bbcli/pi-coding-agent/utils/title-generator";
...
beforeEach(async () => {
	...
	delete process.env.WARP_CLI_AGENT_PROTOCOL_VERSION;
	const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-abortguard-"));
