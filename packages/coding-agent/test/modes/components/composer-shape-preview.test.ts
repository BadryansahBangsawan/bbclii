...
import { COMPOSER_SHAPE_VALUES, type ComposerShape } from "@bbcli/pi-coding-agent/config/settings-schema";
import {
	ComposerShapePreview,
	renderComposerShapePreview,
} from "@bbcli/pi-coding-agent/modes/components/composer-shape-preview";
import {
	getComposerShapeOptions,
	installExtensionComposerShape,
} from "@bbcli/pi-coding-agent/modes/components/composer-shape-registry";
import { SettingsSelectorComponent } from "@bbcli/pi-coding-agent/modes/components/settings-selector";
import { initTheme, setTheme } from "@bbcli/pi-coding-agent/modes/theme/theme";
...
describe("composer shape preview", () => {
	...
	it("borrows status rows from the live status source per shape layout", async () => {
		...
		expect(box).toContain("bbcli"); // stand-in title forwarded to the status source
		...
		expect(borderless).toContain("bbcli");
