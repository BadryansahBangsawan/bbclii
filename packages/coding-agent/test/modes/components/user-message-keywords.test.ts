...
import { UserMessageComponent } from "@bbcli/pi-coding-agent/modes/components/user-message";
import { chipLabel } from "@bbcli/pi-coding-agent/modes/composer-attachments";
import { imageReferenceHyperlink } from "@bbcli/pi-coding-agent/modes/image-references";
...
describe("UserMessageComponent magic-keyword highlighting", () => {
	...
	it("wraps image references in file hyperlinks when a blob path is available", () => {
		const imagePath = path.resolve("/tmp/bbcli-image.png");
	...
	it("renders a video marker as a video chip linked to its source", () => {
		const videoPath = path.resolve("/tmp/bbcli-video.mp4");
