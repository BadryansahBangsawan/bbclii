...
describe("bbcli setup python", () => {
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			projectDir = TempDir.createSync("@bbcli-setup-python-");
			...
			await Bun.write(path.join(cwd, ".bbcli", "config.yml"), `python:\n  interpreter: ${interpreter}\n`);
...
describe("bbcli setup without a component", () => {
	let projectDir: TempDir | undefined;
	...
	for (const flags of [["--check"], ["--json"]]) {
		it(`fails on stderr with a non-zero exit for ${["setup", ...flags].join(" ")}`, async () => {
			projectDir = TempDir.createSync("@bbcli-setup-noarg-");
