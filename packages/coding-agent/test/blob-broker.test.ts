...
import type { AssistantMessage, AssistantMessageEvent, Context, Model } from "@bbcli/pi-ai";
...
describe("BlobRegistry persistence", () => {
	...
	function makePersist(ttlMs: number): BlobPersistence {
		const dir = path.join(os.tmpdir(), `bbcli-blob-registry-${process.pid}-${persistSeq++}`);
...
describe("uploaders", () => {
	...
	it("runs a command uploader end to end against a stub binary", async () => {
		const stub = path.join(os.tmpdir(), `bbcli-test-uploader-${process.pid}.sh`);
		await Bun.write(
			stub,
