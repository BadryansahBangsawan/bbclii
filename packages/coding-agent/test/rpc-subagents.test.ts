...
import { RpcClient } from "@bbcli/pi-coding-agent/modes/rpc/rpc-client";
import {
...
import { RpcSubagentRegistry, readRpcSubagentTranscript } from "@bbcli/pi-coding-agent/modes/rpc/rpc-subagents";
import type { RpcSubagentFrame } from "@bbcli/pi-coding-agent/modes/rpc/rpc-types";
import {
	type AgentProgress,
...
} from "@bbcli/pi-coding-agent/task";
...
describe("readRpcSubagentTranscript", () => {
	test("returns complete JSONL entries and byte cursor", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-rpc-subagent-transcript-"));
		tempPaths.push(dir);
	...
	test("returns empty cursor result for missing transcript files", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-rpc-subagent-transcript-missing-"));
		tempPaths.push(dir);
...
describe("RpcClient subagent frames", () => {
	test("dispatches subagent frames and session-specific events", async () => {
		const scriptPath = path.join(os.tmpdir(), `bbcli-rpc-subagent-client-${Date.now()}.js`);
