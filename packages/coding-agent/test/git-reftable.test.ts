...
describe.skipIf(!supportsReftable)("git reftable support", () => {
	...
	beforeAll(async () => {
		...
		sharedRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-reftable-"));
		...
		worktreeDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-reftable-wt-"));
		...
		configRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-reftable-cfg-"));
