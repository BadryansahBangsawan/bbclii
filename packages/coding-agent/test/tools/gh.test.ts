...
import type { ToolCall } from "@bbcli/pi-ai";
import { toolWireSchema } from "@bbcli/pi-ai/utils/schema";
import { validateToolArguments } from "@bbcli/pi-ai/utils/validation";
...
} from "@bbcli/pi-coding-agent/tools/gh";
import { parseIssueUrl, parsePullRequestUrl } from "@bbcli/pi-coding-agent/tools/gh-common";
import { github } from "@bbcli/pi-coding-agent/utils/github";
import { withRepoLock } from "@bbcli/pi-coding-agent/utils/repo-lock";
import type { VcsGitRepo } from "@bbcli/pi-natives";
import * as vcs from "@bbcli/pi-natives/vcs";
import { getAgentDir, hashPath, normalizePathForComparison, removeWithRetries, setAgentDir } from "@bbcli/pi-utils";
 ...
 * user's real `~/.bbcli/wt`. Returns the temp home and a cleanup hook.
 */
interface TempHome {
	home: string;
...
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	// through the spied homedir instead of `$XDG_DATA_HOME/bbcli/wt` (CI sets these).
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".bbcli", "wt", segment));
}
	...
	describe("pr_checkout (single, cross-repository) and git remote handling", () => {
		...
		describe("git.remote.add idempotency", () => {
			...
			it("does not depend on localized git remote-add stderr for existing remotes", async () => {
				...
				const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-fake-git-"));
...
`,
	...
	it("pins gh messages while preserving UTF-8 character locale", async () => {
		if (process.platform === "win32") return;
		const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-fake-gh-locale-"));
