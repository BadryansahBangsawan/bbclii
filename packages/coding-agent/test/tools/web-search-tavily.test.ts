...
import { searchTavily } from "@bbcli/pi-coding-agent/web/search/providers/tavily";
import type { SearchProviderError } from "@bbcli/pi-coding-agent/web/search/types";
...
describe("Tavily web search provider", () => {
	...
	it("retries recency-filtered empty responses without time_range", async () => {
		...
		const response = await searchTavily({
			...makeParams("BBCLI bbcli latest release notes advisor"),
			numSearchResults: 5,
			recency: "month",
			fetch: fetchMock,
		});
		...
		expect(requestBodies[0]).toMatchObject({
			query: "BBCLI bbcli latest release notes advisor",
			max_results: 5,
			time_range: "month",
		});
