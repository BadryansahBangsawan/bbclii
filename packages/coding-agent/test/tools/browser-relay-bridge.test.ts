...
import { RelayBridge, type RelaySocket } from "@bbcli/pi-coding-agent/tools/browser/relay/bridge";
import type {
	RelayRpcRequest,
	RelayToExtMessage,
	TabSnapshot,
} from "@bbcli/pi-coding-agent/tools/browser/relay/protocol";
...
/**
 * Emulate the bbcli tab worker adopting a tab: attach to its page target, then
...
describe("RelayBridge tab grouping", () => {
	it("groups nothing on hello or tab lifecycle events — only claimed tabs join the bbcli group", () => {
		const bridge = new RelayBridge({ group: { title: "bbcli", color: "cyan" } });
	...
	it("groups exactly the tab a client claims", async () => {
		...
		expect(groups[0]!.title).toBe("bbcli");
	...
	it("never re-groups a tab the user pulled out of the bbcli group", async () => {
		const bridge = new RelayBridge({ group: { title: "bbcli", color: "cyan" } });
		const ext = new FakeExtSocket();
		connect(bridge, ext, [tab({ tabId: 1 })]);
		const cdp = new FakeCdpSocket();
	...
	it("never overlaps group RPCs: a tab claimed mid-flight waits for the pending group", async () => {
		...
		// and mint duplicate "bbcli" groups; the second request must queue.
	...
	it("regroups claimed tabs after an extension reconnect instead of treating the dissolve as user opt-out", async () => {
		...
		ack(bridge, ext, "group", { grouped: { "1": 42 } });
		await flush();
		// Relay/extension link drops: the extension dissolves the bbcli group on
...
describe("RelayBridge attachment release", () => {
	...
	it("still fans root Runtime events out to a session that never enabled the domain", async () => {
		...
		const cdp = new FakeCdpSocket();
		const connId = bridge.cdpConnected(cdp);
		// bbcli's own patched-puppeteer client pull-acquires contexts and never
