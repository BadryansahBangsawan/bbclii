...
import {
	...
	parseLineRanges,
	splitPathAndSel,
} from "@bbcli/pi-coding-agent/tools/path-utils";
...
describe("isFilesystemSourcePath", () => {
	it("accepts POSIX, Windows drive, and UNC paths", () => {
		...
		expect(isFilesystemSourcePath("C:\\repo\\.bbcli\\tools\\systemd.ts")).toBe(true);
		expect(isFilesystemSourcePath("C:/.bbcli/tools/systemd.ts")).toBe(true);
		expect(isFilesystemSourcePath("\\\\server\\share\\.bbcli\\tools\\systemd.ts")).toBe(true);
		expect(isFilesystemSourcePath("//server/share/.bbcli/tools/systemd.ts")).toBe(true);
