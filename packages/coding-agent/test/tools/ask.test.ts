...
import { getThemeByName, initTheme, theme, type Theme } from "@bbcli/pi-coding-agent/modes/theme/theme";
import type { ToolSession } from "@bbcli/pi-coding-agent/tools";
import { AskTool, askToolRenderer } from "@bbcli/pi-coding-agent/tools/ask";
import { ToolAbortError } from "@bbcli/pi-coding-agent/tools/tool-errors";
import { TERMINAL } from "@bbcli/pi-tui";
	...
	it("passes descriptions to the selector while returning selected labels", async () => {
		...
		const select = vi.fn(async (_prompt: string, options: ExtensionUISelectItem[]) => {
			expect(options[0]).toEqual({
				...
				description: "Authenticate with provider keys already configured under ~/.bbcli.",
			});
