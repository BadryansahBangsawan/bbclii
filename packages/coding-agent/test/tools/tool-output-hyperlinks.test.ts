...
import { editToolRenderer } from "@bbcli/pi-coding-agent/edit/renderer";
...
import { astGrepToolRenderer } from "@bbcli/pi-coding-agent/tools/ast-grep";
import { ReadTool, readToolRenderer } from "@bbcli/pi-coding-agent/tools/read";
import { WriteTool, writeToolRenderer } from "@bbcli/pi-coding-agent/tools/write";
...
describe("tool output OSC 8 file:// hyperlinks", () => {
	it("links plain text and image read titles to the resolved filesystem path", async () => {
		...
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-link-read-"));
		try {
	...
	it("links the write header to the absolute path it wrote", async () => {
		settings.override("tui.hyperlinks", "always");
		const theme = (await getThemeByName("dark"))!;
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-link-write-"));
		try {
	...
	it("resolves scoped search links against cwd, not the (sub)scope path", async () => {
		...
		const projectRoot = path.resolve("/tmp/bbcli-project");
	...
	it("links the edit header to the absolute details.path even when the arg path is relative", async () => {
		settings.override("tui.hyperlinks", "always");
		const theme = (await getThemeByName("dark"))!;
		const editPath = path.resolve("/tmp/bbcli-project/src/a.ts");
