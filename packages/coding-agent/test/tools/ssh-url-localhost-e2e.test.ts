...
import * as capability from "@bbcli/pi-coding-agent/capability";
import type { SSHHost } from "@bbcli/pi-coding-agent/capability/ssh";
import type { CapabilityResult } from "@bbcli/pi-coding-agent/capability/types";
...
import { InternalUrlRouter } from "@bbcli/pi-coding-agent/internal-urls/router";
import { SshProtocolHandler } from "@bbcli/pi-coding-agent/internal-urls/ssh-protocol";
...
describe.skipIf(!SSH_OK)("ssh:// handler against a real localhost ssh", () => {
	...
	const TMP = `/tmp/bbcli-ssh-e2e-${process.pid}`;
	...
	it("writes byte-exact, leaves no temp, and the read path round-trips", async () => {
		...
		const leftovers = await Bun.$`ssh -o BatchMode=yes localhost ls ${TMP} | grep -c bbcli-tmp || true`.text();
...
describe.skipIf(!SSH_OK)("ssh:// through the real read/grep/write tools (localhost)", () => {
	const TMP = `/tmp/bbcli-ssh-tools-e2e-${process.pid}`;
	...
	it("GrepTool reports matches under the ssh:// URL with no scratch-temp leak", async () => {
		...
		expect(out).not.toContain("bbcli-search-probe");
		expect(detailsJson).not.toContain("bbcli-search-probe");
	});
