...
} from "@bbcli/pi-coding-agent/internal-urls";
...
import * as sshFileTransfer from "@bbcli/pi-coding-agent/ssh/file-transfer";
...
describe("GrepTool internal URL resolution", () => {
	...
	it("expands bbcli:// root to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# bbcli://tools/grep.md");
		expect(text).toContain("Grep file contents with a regex across files");
	});

	it("expands bbcli://docs to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# bbcli://tools/read.md");
