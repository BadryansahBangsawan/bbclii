...
import type { UserMessage } from "@bbcli/pi-ai";
...
import { BUILTIN_MODE_SLASH_COMMANDS } from "@bbcli/pi-coding-agent/slash-commands/builtin-modes";
...
describe("skillful setting and /skillful session toggle", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tempDir, ".bbcli", "skills", "test-skill"), { recursive: true });
		fs.writeFileSync(
			path.join(tempDir, ".bbcli", "skills", "test-skill", "SKILL.md"),
