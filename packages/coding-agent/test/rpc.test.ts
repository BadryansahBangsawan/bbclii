...
import type { AgentEvent, AgentMessage } from "@bbcli/pi-agent-core";
import { type AssistantMessage, Effort, type TextContent } from "@bbcli/pi-ai";
import {
...
import type { BashExecutionMessage } from "@bbcli/pi-coding-agent/session/messages";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = path.join(os.tmpdir(), `bbcli-rpc-test-${Snowflake.next()}`);
...
describe("RPC fast mode with unsupported Fireworks model and priority tier", () => {
	...
	beforeEach(async () => {
		sessionDir = path.join(os.tmpdir(), `bbcli-rpc-fast-mode-test-${Snowflake.next()}`);
