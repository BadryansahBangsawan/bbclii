...
import { listAllSessions, resolveResumableSession } from "@bbcli/pi-coding-agent/session/session-listing";
import { computeDefaultSessionDir } from "@bbcli/pi-coding-agent/session/session-paths";
import { FileSessionStorage } from "@bbcli/pi-coding-agent/session/session-storage";
import { executeBuiltinSlashCommand } from "@bbcli/pi-coding-agent/slash-commands/builtin-registry";
import { getConfigRootDir, refreshDirsFromEnv, setAgentDir } from "@bbcli/pi-utils";
...
beforeEach(async () => {
	tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-resume-command-"));
...
describe("/resume slash command", () => {
	...
	it.skipIf(process.platform === "win32")("lists and resumes sessions stored in XDG_DATA_HOME", async () => {
		...
		const xdgOmpDir = path.join(xdgDataDir, "bbcli");
