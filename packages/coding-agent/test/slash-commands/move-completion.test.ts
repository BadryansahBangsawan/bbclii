...
import { BUILTIN_SLASH_COMMANDS } from "@bbcli/pi-coding-agent/slash-commands/builtin-registry";
...
describe("/move directory completion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-move-completion-"));
	...
	it("completes parent directory paths", async () => {
		...
		const siblingName = `bbcli-move-sibling-${path.basename(tempDir)}`;
