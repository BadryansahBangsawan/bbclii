...
import { discoverAndLoadExtensions } from "@bbcli/pi-coding-agent/extensibility/extensions/loader";
import { getAgentDir, getPluginsDir, removeSyncWithRetries, setAgentDir, TempDir } from "@bbcli/pi-utils";

const currentPiCodingAgentPath = Bun.resolveSync("@bbcli/pi-coding-agent", import.meta.dir);
const currentPiExtensionsPath = Bun.resolveSync("@bbcli/pi-coding-agent/extensibility/extensions", import.meta.dir);
...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		...
		// resolves into `<tempHome>/.bbcli/plugins` on every platform. Two things are needed:
		//  - mock os.homedir() so configRoot = `<tempHome>/.bbcli` (the previous
		//    XDG_DATA_HOME redirect was a no-op on Windows, where these tests then wrote
		//    into and rm'd the developer's real `~/.bbcli/plugins`);
		//  - clear the XDG_* vars, because on Linux/macOS the resolver prefers
		//    `$XDG_DATA_HOME/bbcli` over the home config root when that dir exists, so an
		...
		// ~/.bbcli must fail loudly here instead of clobbering the developer's plugins.
	...
	it("resolves a sub-extension directory via its own package.json manifest over a decoy index", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "feature-ext", version: "1.0.0", bbcli: { extensions: ["./dist/real-ext.ts"] } }),
		);
		fs.writeFileSync(
			realEntry,
			[
