...
/**
 * Swap `@bbcli/pi-utils` and the changelog module's `../config` import for a
...
describe("bundled changelog asset path resolution", () => {
	const moduleUrl = new URL("file:///opt/bbcli/dist/cli.js");

	test.each([
		["Windows drive-letter", String.raw`C:\bbcli\dist\CHANGELOG.md`],
		["Windows UNC", String.raw`\\server\share\bbcli\CHANGELOG.md`],
		["POSIX", "/opt/bbcli/dist/CHANGELOG.md"],
...
describe("changelog static import resources", () => {
	...
	test("reads the emitted changelog asset when run outside the bundle directory", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-changelog-bundle-"));
		try {
			...
			const buildOutput = await Bun.build({
				...
				outdir: bundleDir,
				target: "bun",
				external: ["bbcli-legacy-pi-modules"],
	...
	test("reads the emitted changelog asset from a compiled binary", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-changelog-compiled-"));
		try {
