...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@bbcli/pi-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.bbcli/`), plus
//    the legacy non-XDG `~/.bbcli/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.bbcli/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/bbcli/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.bbcli` when
...
// symlinks `~/.bbcli/agent/extensions/foo.ts` to a dotfiles repo would see the
