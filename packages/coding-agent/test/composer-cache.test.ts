...
import { COMPOSER_DEFAULTS, type ComposerStatusSnapshot } from "@bbcli/pi-coding-agent/modes/composer";
import {
...
} from "@bbcli/pi-coding-agent/modes/composer-cache";
import { getComposerCacheDir } from "@bbcli/pi-utils/dirs";

describe("composer startup cache", () => {
	it("round-trips per-project UI, status, recent-session JSONL, and LSP speculation", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-composer-cache-"));
	...
	it("ignores legacy status snapshots", async () => {
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-composer-cache-legacy-status-"));
		const key = Bun.hash.wyhash(path.resolve(cwd)).toString(16).padStart(16, "0");
		const cacheDir = path.join(getComposerCacheDir(), key);
		try {
			await Bun.write(
	...
	it("loads XDG_CACHE_HOME from the home .env before the first cache access", async () => {
		...
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-composer-cache-dotenv-"));
		...
		try {
			await Promise.all([
				...
				fs.mkdir(path.join(xdgCache, "bbcli"), { recursive: true }),
			]);
			...
			const script = [
				...
				`const expected = path.join(${JSON.stringify(xdgCache)}, "bbcli", "cache", "composer", key, "welcome.json");`,
