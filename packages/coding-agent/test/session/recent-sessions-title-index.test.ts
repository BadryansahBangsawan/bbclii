...
import { getRecentSessions } from "@bbcli/pi-coding-agent/session/session-listing";
import { SessionManager } from "@bbcli/pi-coding-agent/session/session-manager";
import { resetSessionTitleIndexForTests } from "@bbcli/pi-coding-agent/session/title-index";
...
describe("recent sessions title index", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-recent-titles-"));
