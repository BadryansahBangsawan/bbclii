...
import type { ResetCreditAccountStatus, ResetCreditRedeemOutcome, ResetCreditTarget, UsageReport } from "@bbcli/pi-ai";
...
describe("wave 5 — adapters and polish", () => {
	...
	it("/ssh add foo --host x --user y --scope user: calls addSSHHost", async () => {
		const sshModule = await import("@bbcli/pi-coding-agent/ssh/config-writer");
	...
	it("/marketplace discover: output is bulleted with '  - ' token", async () => {
		const { MarketplaceManager } = await import("@bbcli/pi-coding-agent/extensibility/plugins/marketplace");
