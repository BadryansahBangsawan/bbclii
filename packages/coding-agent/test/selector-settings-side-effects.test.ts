...
import { MODEL_ROLE_IDS } from "@bbcli/pi-coding-agent/config/model-roles";
...
import { ReadToolGroupComponent } from "@bbcli/pi-coding-agent/modes/components/read-tool-group";
import { ToolExecutionComponent } from "@bbcli/pi-coding-agent/modes/components/tool-execution";
import { SelectorController } from "@bbcli/pi-coding-agent/modes/controllers/selector-controller";
...
import type { ResolvedRoleModel } from "@bbcli/pi-coding-agent/session/agent-session";
import { AUTO_THINKING } from "@bbcli/pi-coding-agent/thinking";
import { setTerminalHyperlinks, TERMINAL } from "@bbcli/pi-tui";
...
describe("selector setting side effects", () => {
	...
	it("switches a global edit when a byte-identical startup runtime override shadows the project default", async () => {
		...
		fs.mkdirSync(path.join(projectDir, ".bbcli"), { recursive: true });
		fs.writeFileSync(path.join(projectDir, ".bbcli", "config.yml"), `modelRoles:\n  default: ${projectSelector}\n`);
	...
	it("persists project and global defaults shadowed by a config overlay without switching", async () => {
		...
		try {
			...
			try {
				...
				expect(settings.getModelRole("default")).toBe(overlaySelector);
				expect(settings.getModelRoleProvenance("default")).toBe("overlay");
				expect(await Bun.file(path.join(projectDir, ".bbcli", "config.yml")).text()).toContain(
					`default: ${projectSelector}`,
				);
				expect(setModel).not.toHaveBeenCalled();
