/**
 * Regression test for #1496 (bug 2): `bbcli install ./my-extension` used to be
...
import { commands, isSubcommand, resolveCliArgv } from "@bbcli/pi-coding-agent/cli-commands";
import { looksLikeLocalPath } from "@bbcli/pi-coding-agent/commands/install";
...
describe("install command is registered as a top-level subcommand", () => {
	...
	test("CLI runner rejects only bare reserved management words", () => {
		expect(resolveCliArgv(["extensions"])).toEqual({
			error: '`bbcli extensions` is not a management command. Use `bbcli plugin list` / `bbcli plugin install`, or run `bbcli launch extensions` if you meant to send "extensions" as a prompt.',
		});
...
describe("looksLikeLocalPath", () => {
	...
	test("npm specs and marketplace refs are remote", () => {
		expect(looksLikeLocalPath("@bbcli/exa")).toBe(false);
	...
	test("bare names that exist as a local directory are treated as local", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-install-test-"));
		try {
