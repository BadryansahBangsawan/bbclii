 ...
 *
 * Both OMP-native loaders are covered: `.bbcli/mcp.json` (native provider) and a
...
beforeEach(async () => {
	...
	tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-reqid-home-"));
	tempAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-reqid-agent-"));
	tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-reqid-cwd-"));
...
test("requestIdFormat from .bbcli/mcp.json reaches the transport config", async () => {
	const configs = await loadFrom(path.join(".bbcli", "mcp.json"), {
