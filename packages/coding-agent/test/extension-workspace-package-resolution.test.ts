...
import { __rewriteLegacyExtensionSourceForTests } from "@bbcli/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
test("bare workspace-member imports resolve through the workspace root manifest", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-workspace-resolve-"));
	tempRoots.push(root);
...
test("installed node_modules copies shadow workspace members at the same level", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-workspace-shadow-"));
	tempRoots.push(root);
