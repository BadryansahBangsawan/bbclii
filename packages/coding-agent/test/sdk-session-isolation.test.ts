...
import { LocalProtocolHandler } from "@bbcli/pi-coding-agent/internal-urls/local-protocol";
...
import * as secrets from "@bbcli/pi-coding-agent/secrets";
...
import { VibeSessionRegistry } from "@bbcli/pi-coding-agent/vibe/runtime";
import { getSessionsDir, removeSyncWithRetries, Snowflake } from "@bbcli/pi-utils";
import { getActiveProfile, getConfigRootDir, setProfile } from "@bbcli/pi-utils/dirs";
...
async function withTempConfigRoot<T>(run: () => Promise<T>): Promise<T> {
	...
	const configDirName = `.bbcli-sdk-session-${Snowflake.next()}`;
...
describe("createAgentSession session storage isolation", () => {
	...
	it("loads configured secrets per session alongside built-in credential redaction", async () => {
		await withClearedSecretEnv(async () => {
			...
			fs.mkdirSync(path.join(cwd, ".bbcli"), { recursive: true });
			fs.writeFileSync(path.join(cwd, ".bbcli", "secrets.yml"), `- type: plain\n  content: ${configuredSecret}\n`);
	...
	it("restores keyed assistant placeholders across reloads", async () => {
		await withClearedSecretEnv(async () => {
			await withTempConfigRoot(async () => {
				...
				fs.mkdirSync(path.join(cwd, ".bbcli"), { recursive: true });
				fs.writeFileSync(
					path.join(cwd, ".bbcli", "secrets.yml"),
