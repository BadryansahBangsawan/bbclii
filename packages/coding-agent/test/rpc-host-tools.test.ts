...
import type { AgentEvent } from "@bbcli/pi-agent-core";
import { defineRpcClientTool, RpcClient } from "@bbcli/pi-coding-agent/modes";
import { RpcHostToolBridge } from "@bbcli/pi-coding-agent/modes/rpc/host-tools";
import type {
	RpcHostToolCallRequest,
	RpcHostToolCancelRequest,
	RpcHostToolUpdate,
} from "@bbcli/pi-coding-agent/modes/rpc/rpc-types";
...
describe("RpcClient custom tools", () => {
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `bbcli-rpc-host-tools-${Date.now()}.js`);
