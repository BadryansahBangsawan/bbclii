...
import { runRootCommand } from "@bbcli/pi-coding-agent/main";
import { describeAuthBrokerStartupError } from "@bbcli/pi-coding-agent/session/auth-broker-config";
import { setInteractiveHost } from "@bbcli/pi-utils";
...
describe("describeAuthBrokerStartupError", () => {
	it("turns a broker connection failure into recovery guidance", async () => {
		...
		expect(message).toContain("bbcli auth-broker serve");
		expect(message).toContain("bbcli config reset auth.broker.url");
...
describe("runRootCommand — unreachable auth broker at startup", () => {
	it("exits 1 with an actionable message instead of an uncaught AuthBrokerError", async () => {
		...
		expect(stderr).toContain("bbcli auth-broker serve");
	}, 15_000);
});
