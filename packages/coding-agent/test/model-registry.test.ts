...
import { Effort, type FetchImpl, type Model, type OpenAICompat, type ThinkingConfig } from "@bbcli/pi-ai";
import { buildModel } from "@bbcli/pi-catalog/build";
import { writeModelCache } from "@bbcli/pi-catalog/model-cache";
import { fingerprintStaticModels } from "@bbcli/pi-catalog/model-manager";
import { calculateUsageCost, getBundledModels } from "@bbcli/pi-catalog/models";
import { finalizeCustomModel } from "@bbcli/pi-coding-agent/config/custom-models";
import { applyModelPatch } from "@bbcli/pi-coding-agent/config/model-patch";
...
describe("ModelRegistry", () => {
	...
	describe("baseUrl override (no custom models)", () => {
		...
		test("refresh keeps transport override on built-in provider (#2555 openrouter gateway)", async () => {
			// Reporter ran `bbcli` with the auth-gateway broker proxying OpenRouter.
