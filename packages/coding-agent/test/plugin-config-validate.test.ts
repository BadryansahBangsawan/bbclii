/**
 * Regression test for `bbcli plugin config validate` (#9106).
 *
...
import type { InstalledPluginSummary } from "@bbcli/pi-coding-agent/extensibility/plugins/marketplace";
import { MarketplaceManager } from "@bbcli/pi-coding-agent/extensibility/plugins/marketplace";
import type { InstalledPlugin } from "@bbcli/pi-coding-agent/extensibility/plugins/types";
...
describe("runPluginCommand({ action: 'config', args: ['validate'] })", () => {
	...
	test("reports an invalid stored value on a marketplace plugin that list() omits", async () => {
		const summary: InstalledPluginSummary = {
			id: "bbcli-commit@market",
			scope: "user",
			entries: [
				{
					scope: "user",
					installPath: "/cache/bbcli-commit",
					version: "2.0.0",
		...
		expect(getPlugin).toHaveBeenCalledWith("bbcli-commit", { path: "/cache/bbcli-commit" });
		...
		expect(report.errors).toContainEqual(expect.objectContaining({ plugin: "bbcli-commit", key: "splitMode" }));
	});
	...
	test("validates against the active project schema over a same-named user plugin", async () => {
		...
		expect(getPlugin).toHaveBeenCalledWith("bbcli-commit");
		const report = JSON.parse(output.join("\n")) as {
			valid: boolean;
			errors: Array<{ plugin: string; key: string }>;
		};
