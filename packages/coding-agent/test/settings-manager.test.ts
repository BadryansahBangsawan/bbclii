...
import { Effort } from "@bbcli/pi-ai";
...
import { __providerInFlightForTesting, streamSimple } from "@bbcli/pi-ai/stream";
import type { Context } from "@bbcli/pi-ai/types";
import {
	...
	type SettingPath,
	Settings,
} from "@bbcli/pi-coding-agent/config/settings";
import * as discovery from "@bbcli/pi-coding-agent/discovery";
import { AgentStorage } from "@bbcli/pi-coding-agent/session/agent-storage";
import { AUTO_IMAGE_PROVIDER_ORDER } from "@bbcli/pi-coding-agent/tools/image-providers";
import { SEARCH_PROVIDER_ORDER } from "@bbcli/pi-coding-agent/web/search/types";
import { getProjectAgentDir, TempDir } from "@bbcli/pi-utils";
import * as fileLock from "@bbcli/pi-utils/file-lock";
...
describe("Settings", () => {
	...
	describe("config file failure safety", () => {
		...
		it("backs up a corrupted project config and retains the pending project role for retry", async () => {
			await writeSettings({});
			const projectConfigPath = path.join(projectDir, ".bbcli", "config.yml");
			await Bun.write(
				projectConfigPath,
		...
		it("observes both failures when global and project configs are malformed", async () => {
			...
			await Promise.all([
				...
				Bun.write(path.join(projectDir, ".bbcli", "config.yml"), malformed),
			]);
			...
			try {
				...
				expect(
					fs.readdirSync(path.join(projectDir, ".bbcli")).some(name => name.startsWith("config.yml.broken-")),
				).toBe(true);
			} finally {
	...
	describe("model role overrides", () => {
		...
		it("preserves concurrent external per-role edits when saving one global role", async () => {
			...
			// External edit (another bbcli instance / manual edit): changes advisor,
