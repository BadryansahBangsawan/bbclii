...
import type { StatusLineSegmentId } from "@bbcli/pi-coding-agent/config/settings-schema";
...
import { getSessionAccentAnsi, getSessionAccentHex } from "@bbcli/pi-coding-agent/utils/session-color";
...
describe("session_name preview-title fallback", () => {
	it("renders the stand-in title when the session is unnamed", () => {
		const seg = renderSegment("session_name", createCtx({ previewTitle: "bbcli" }));
		expect(seg.visible).toBe(true);
		expect(stripAnsi(seg.content)).toBe("bbcli");
	});

	it("prefers the real session name over the stand-in", () => {
		const seg = renderSegment("session_name", createCtx({ sessionName: "Named session", previewTitle: "bbcli" }));
	...
	it("right-aligns the stand-in title through the box border pipeline", () => {
		...
		const withTitle = component.getTopBorder(80, "bbcli");
		// The gauge fill pads the group gap, so the title chip lands flush right.
		expect(withTitle.width).toBe(80);
		expect(stripAnsi(withTitle.content).trimEnd().endsWith("bbcli")).toBe(true);
		// Live render path passes no preview title: unnamed sessions show none.
		expect(stripAnsi(component.getTopBorder(80).content)).not.toContain("bbcli");
	});
});
...
describe("path segment truncation at varying maxLength", () => {
	...
	beforeAll(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-overflow-very-long-directory-name-for-testing-"));
...
describe("overflow: path shrinks before git is dropped", () => {
	...
	beforeAll(() => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-overflow-a-very-long-worktree-directory-name-here-"));
		setProjectDir(tmpDir);
	});
	...
	it("shrinks a short path when maxLength exceeds actual path length", () => {
		...
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-short-"));
	...
	it("preserves git when overflow is only 1-2 columns", () => {
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-narrow-ovf-"));
		setProjectDir(shortDir);
		try {
...
describe("overflow: path survives before model", () => {
	it("drops the model segment before the cwd path when both cannot fit", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-statusline-overflow-"));
