...
import { clearCache, readFile } from "@bbcli/pi-coding-agent/capability/fs";
...
import { getMCPConfigPath, getProjectDir, removeWithRetries, setProjectDir } from "@bbcli/pi-utils";
...
describe("/mcp reload picks up external mcp.json edits", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-reload-project-"));
