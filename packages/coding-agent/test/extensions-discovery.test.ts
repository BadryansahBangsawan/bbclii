...
import { type ExtensionModule, extensionModuleCapability } from "@bbcli/pi-coding-agent/capability/extension-module";
import { resetSettingsForTest, Settings } from "@bbcli/pi-coding-agent/config/settings";
import { getCapability, initializeWithSettings } from "@bbcli/pi-coding-agent/discovery";
import {
	discoverAndLoadExtensions,
	discoverExtensionPaths,
	loadExtensions,
} from "@bbcli/pi-coding-agent/extensibility/extensions/loader";
import { discoverSessionExtensionPaths } from "@bbcli/pi-coding-agent/sdk";
...
describe("extensions discovery", () => {
	...
	it("discovers a symlinked extension directory with a package.json manifest", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "ctk", bbcli: { extensions: ["./index.ts"] } }),
		);
