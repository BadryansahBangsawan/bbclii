...
describe("SSH control-path budget (#9070)", () => {
	it("rejects a control dir that overflows sun_path once %C.sock + mux temp bind is added", () => {
		...
		const profileDir = "/Users/arthur/.bbcli/profiles/upstream/ssh-control";
		...
		expect(controlPathFitsBudget("/Users/arthur/.bbcli/ssh-control", "darwin")).toBe(true);
	});
...
describe("sshControlFallbackDir", () => {
	it("is deterministic and leaves 11 bytes of macOS sun_path slack", () => {
		const canonicalDir = "/Users/arthur/.bbcli/profiles/upstream/ssh-control";
		const a = sshControlFallbackDir(canonicalDir, 501);
		const b = sshControlFallbackDir(canonicalDir, 501);
		expect(a).toBe(b);
		expect(a).toBe("/tmp/bbcli-5434354bc38f9a50fbbd");
	...
	it("isolates distinct canonical control directories and uids", () => {
		const base = "/Users/arthur/.bbcli/ssh-control";
		expect(sshControlFallbackDir(base, 501)).not.toBe(
			sshControlFallbackDir("/different/xdg/state/bbcli/ssh-control", 501),
		);
...
describe("resolveSshControlDir", () => {
	it("keeps the canonical dir when it fits", () => {
		const canonicalDir = "/Users/arthur/.bbcli/ssh-control";
	...
	it("relocates to the bounded shared fallback when the canonical dir overflows", () => {
		...
		expect(choice).toEqual({ dir: "/tmp/bbcli-5434354bc38f9a50fbbd", shared: true });
	...
	it("keeps distinct fallback masters for the same profile under different XDG state roots", () => {
		const a = resolveSshControlDir({
			canonicalDir: "/very/long/xdg-state-a/bbcli/profiles/upstream/ssh-control",
			platform: "darwin",
			uid: 501,
		});
		const b = resolveSshControlDir({
			canonicalDir: "/very/long/xdg-state-b/bbcli/profiles/upstream/ssh-control",
			platform: "darwin",
			uid: 501,
		});
...
describe("assertOwnerPrivateDir", () => {
	...
	const mkScratch = () => {
		scratch = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-ssh-guard-"));
		return scratch;
	};
