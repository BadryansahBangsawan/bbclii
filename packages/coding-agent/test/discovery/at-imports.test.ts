...
import { expandAtImports, MAX_AT_IMPORT_DEPTH } from "@bbcli/pi-coding-agent/discovery/at-imports";
...
describe("expandAtImports", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-at-import-"));
	});
	...
	test("resolves ~/path against the home override", async () => {
		const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-at-home-"));
		try {
