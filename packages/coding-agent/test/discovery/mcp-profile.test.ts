 ...
 *
 * A named profile relocates the agent directory to ~/.bbcli/profiles/<name>/agent.
 * The native config provider used to read user-scope mcp.json from the literal
 * home (~/.bbcli/agent/mcp.json) via `ctx.home`, so a profile never saw its own
 ...
 *
 * `os.homedir()` is mocked so the *old* code path (ctx.home + ".bbcli/agent")
...
import { type MCPServer, mcpCapability } from "@bbcli/pi-coding-agent/capability/mcp";
import { loadCapability } from "@bbcli/pi-coding-agent/discovery";
import { getConfigRootDir, removeWithRetries, setAgentDir } from "@bbcli/pi-utils";
...
describe("native user-level MCP discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-profile-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-profile-project-"));
	...
	test("active profile loads its own user server, not the default profile's", async () => {
		// Active profile's agent dir (stand-in for ~/.bbcli/profiles/<name>/agent).
		const profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-mcp-profile-agent-"));
		...
		await writeMcpJson(path.join(tempHome, ".bbcli", "agent"), {
	...
	test("default profile loads the user server from ~/.bbcli/agent", async () => {
		const defaultAgentDir = path.join(tempHome, ".bbcli", "agent");
