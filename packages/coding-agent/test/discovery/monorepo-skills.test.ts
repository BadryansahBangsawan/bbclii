...
import type { Skill } from "@bbcli/pi-coding-agent/capability/skill";
import type { LoadContext, LoadResult } from "@bbcli/pi-coding-agent/capability/types";
import { scanSkillsFromDir } from "@bbcli/pi-coding-agent/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	test("finds skills in ancestor .bbcli/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".bbcli", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".bbcli", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".bbcli", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".bbcli", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".bbcli", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".bbcli", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".bbcli", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".bbcli", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".bbcli", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".bbcli", "skills"), "root-skill", "At repo root");
