...
import {
	...
	type Rule,
	ruleCapability,
} from "@bbcli/pi-coding-agent/capability/rule";
import type { LoadContext } from "@bbcli/pi-coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@bbcli/pi-coding-agent/discovery";
import { TtsrManager, type TtsrMatchContext } from "@bbcli/pi-coding-agent/export/ttsr";
...
describe("builtin-defaults rule provider", () => {
	...
	it("fires ts-no-local-is-record on local function and lambda definitions", async () => {
		...
		manager.resetBuffer();
		expect(
			manager.checkDelta('import { isRecord } from "@bbcli/pi-utils";', {
				source: "tool",
				toolName: "write",
				filePaths: ["packages/x/src/foo.ts"],
			}),
