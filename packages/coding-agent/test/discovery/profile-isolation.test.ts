 ...
 * profile. A profile relocates the agent directory to ~/.bbcli/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.bbcli/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".bbcli/agent") points at the tempdir decoys below; without the fix
...
import { type Skill, skillCapability } from "@bbcli/pi-coding-agent/capability/skill";
import { type SlashCommand, slashCommandCapability } from "@bbcli/pi-coding-agent/capability/slash-command";
...
describe("native user-level config discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-profile-iso-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-profile-iso-project-"));
		profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-profile-iso-agent-"));
