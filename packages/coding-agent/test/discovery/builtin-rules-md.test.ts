 ...
 * from both `~/.bbcli/agent/RULES.md` (user) and the nearest `.bbcli/RULES.md`
...
import { getCapability } from "@bbcli/pi-coding-agent/capability";
import { clearCache } from "@bbcli/pi-coding-agent/capability/fs";
import { type Rule, ruleCapability } from "@bbcli/pi-coding-agent/capability/rule";
...
import { loadCapability } from "@bbcli/pi-coding-agent/discovery";
...
test("user ~/.bbcli/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".bbcli", "agent", "RULES.md"),
...
test("project .bbcli/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".bbcli", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".bbcli", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".bbcli", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".bbcli", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".bbcli", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".bbcli", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("enabled false omits a discovered rule", async () => {
	const rulesDir = path.join(home, ".bbcli", "agent", "rules");
	writeFile(
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .bbcli/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".bbcli", "agent", "rules", "other.md"), "# Unrelated rule\n");
