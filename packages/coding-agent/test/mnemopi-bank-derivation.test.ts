...
import { computeMnemopiBankScope, extendRecallWithLegacyBanks } from "@bbcli/pi-coding-agent/mnemopi/config";
import { removeWithRetries, TempDir } from "@bbcli/pi-utils";

// Set up a fixture filesystem we can reuse across the two regression
// suites — same shape as `~/.bbcli/memories/mnemopi/` on a real install.
let rootDir: TempDir;
let dbDir: string;
let banksDir: string;
...
describe("computeMnemopiBankScope (#2412)", () => {
	...
	it("returns the same per-project bank for one cwd regardless of git state", async () => {
		...
		try {
			const project = baseDir.join("projects", "bbcli-workstation");
