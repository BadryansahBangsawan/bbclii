 ...
 * bbcli's status-band brand fade): idle renders the bbcli icon in the dim color;
...
describe("status line brand fade", () => {
	it("fades the brand from dim into the accent when a turn starts", () => {
		...
		try {
			// Idle: bbcli icon settled in the dim color.
			expect(component.renderBottomBar(80, "full")).toContain(`${dimAnsi}${theme.icon.bbcli}`);
			...
			expect(early).not.toContain(theme.icon.bbcli);
	...
	it("fades back to dim from the on-screen accent when the turn ends", () => {
		...
		try {
			...
			expect(ending).toContain(theme.icon.bbcli);
