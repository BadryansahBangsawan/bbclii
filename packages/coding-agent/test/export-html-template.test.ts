...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-html-template-"));
...
const bundledDependencyStubs: Record<string, string> = {
	"@bbcli/pi-utils": 'export const APP_NAME = "bbcli"; export const isEnoent = () => false;',
	"../../modes/theme/theme":
...
describe("HTML export template", () => {
	...
	test("composes the exact source template once and returns the cached bytes", async () => {
		...
		expect(first).toContain("const THEME_STORAGE_KEY = 'bbcli-export-theme';");
