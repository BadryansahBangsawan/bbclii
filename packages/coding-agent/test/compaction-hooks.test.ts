...
import {
	...
	type SessionEvent,
} from "@bbcli/pi-coding-agent/extensibility/hooks";
import { theme } from "@bbcli/pi-coding-agent/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `bbcli-compaction-hooks-test-${Snowflake.next()}`);
