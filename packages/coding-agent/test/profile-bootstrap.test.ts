...
describe("extractProfileFlags", () => {
	...
	it("does not eat the value of known string-valued flags", () => {
		// `bbcli --system-prompt --profile foo` must pass the literal `--profile`
	...
	it("does not eat the value of --approval-mode", () => {
		...
		// `bbcli --approval-mode --profile foo` silently activates profile `foo`
	...
	it("keeps the built-in --plan from swallowing the profile boundary when its extension is absent", () => {
		...
		// become "--bbcli-profile-boundary" and the user's message would be dropped.
		const extracted = extractProfileFlags(["--plan", "--profile", "work", "follow up"]);
	...
	it("stops extracting global flags at a subcommand boundary", () => {
		// `bbcli grep --profile <path>` must reach the grep subcommand intact; the
	...
	it("treats explicit launch as the default command and keeps extracting globals", () => {
		expect(extractProfileFlags(["launch", "--profile", "work", "--alias", "bbcli-work"])).toEqual({
			argv: ["launch"],
			profile: "work",
	...
	it("exempts known value-less launch flags so a trailing profile still activates", () => {
		...
		// the token after them is a fresh argument: `bbcli --print --profile work`
	...
	it("does not hide a global --profile/--alias behind an unknown flag with a flag-looking successor", () => {
		...
		// consume value-like (non-`-`) successors. So `bbcli --some-ext-flag --profile
		...
		expect(extractProfileFlags(["--some-ext-flag", "--alias", "bbcli-work"])).toEqual({
			argv: ["--some-ext-flag"],
			profile: undefined,
			aliasName: "bbcli-work",
		});
