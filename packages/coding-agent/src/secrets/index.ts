...
import { SENSITIVE_TOKEN_RE } from "@bbcli/pi-ai/providers/transform-messages";
import { getSecretPlaceholderKeyPath, isEnoent, logger } from "@bbcli/pi-utils";
 ...
 * — `$XDG_STATE_HOME/bbcli/secret-placeholder.key` (or `~/.bbcli/agent/secret-placeholder.key`
...
export async function loadSecrets(cwd: string, agentDir: string): Promise<SecretEntry[]> {
	const projectPath = path.join(cwd, ".bbcli", "secrets.yml");
