...
import { getProjectDir, prompt } from "@bbcli/pi-utils";
import {
...
export async function loadSkills(options: LoadSkillsOptions = {}): Promise<LoadSkillsResult> {
	...
	function isSourceEnabled(source: SourceMeta): boolean {
		...
		// User-scope claude-plugins skills carry the root's origin (#10743). bbcli's
		// own installs (`bbcli` registry, `--plugin-dir`) are not the foreign
