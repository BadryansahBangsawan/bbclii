...
import { getMCPConfigPath, logger } from "@bbcli/pi-utils";
...
async function handleSmitherySearchCommand(rest: string, runtime: SlashCommandRuntime): Promise<SlashCommandResult> {
	...
	try {
		...
		if (AIError.is(AIError.classify(err), AIError.Flag.AuthFailed)) {
			return usage(
				"Smithery authentication required. Run /mcp smithery-login in the TUI client or add an API key to ~/.bbcli/agent/smithery.json.",
				runtime,
			);
		}
