...
import { EventLoopKeepalive, type ThinkingLevel } from "@bbcli/pi-agent-core";
import type { ImageContent, Model } from "@bbcli/pi-ai";
import {
	$env,
	directoryIsMissing,
	getLogPath,
...
import { injectOmpExtensionCliRoots } from "./discovery/bbcli-extension-roots";
...
async function runInteractiveMode(
	...
	// `bbcli join <link>`: dispatch through the same builtin path as a typed
 ...
 * via `--model`, and listed by `bbcli models find`. Once the initial refresh settles,
	...
	if (parsed.fork) {
		...
		if (!match) {
			throw new SessionResolutionError(
				...
				"Run `bbcli --resume` without an argument to pick from recent sessions, or `bbcli` to start a new one.",
			);
		}
...
function discoverSystemPromptFile(): string | undefined {
	// Check project-local first (.bbcli/SYSTEM.md, .pi/SYSTEM.md legacy)
...
export async function runRootCommand(
	...
	try {
		...
		if (!parsedArgs.trustedExtensions?.length) {
			...
			// the `bbcli-plugins` discovery provider can surface their `skills/`, `hooks/`,
		...
		if (mode === "acp") {
			...
			// Fail fast on stale/typo flags (e.g. `bbcli --list-models`) now that we
