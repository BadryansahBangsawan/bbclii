 ...
 * SSH JSON Provider
 *
 * Discovers SSH hosts from managed bbcli config paths and legacy root ssh.json files.
 * Priority: 5 (low, project/user config discovery)
 */
import * as path from "node:path";
import { getSSHConfigPath, tryParseJson } from "@bbcli/pi-utils";
...
registerProvider(sshCapability.id, {
	...
	description: "Load SSH hosts from managed bbcli paths and legacy ssh.json/.ssh.json files",
	priority: 5,
	load,
});
