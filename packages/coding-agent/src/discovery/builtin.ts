/**
 * Builtin Provider (.bbcli)
 *
 * Primary provider for OMP native configs. Supports all capabilities.
 */
import * as path from "node:path";
import { getAgentDir, logger, parseFrontmatter, tryParseJson } from "@bbcli/pi-utils";
...
const DESCRIPTION = "Native OMP configuration from ~/.bbcli and .bbcli/";
const PRIORITY = 100;
...
async function getConfigDirs(ctx: LoadContext): Promise<Array<{ dir: string; level: "user" | "project" }>> {
	...
	// profile's agent dir (~/.bbcli/profiles/<name>/agent), like sessions and MCP.
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .bbcli/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.bbcli/agent/skills/
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "Auto-generated managed skills from ~/.bbcli/agent/managed-skills",
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// User scope:    ~/.bbcli/agent/RULES.md
	// Project scope: nearest .bbcli/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .bbcli/ directories",
	priority: PRIORITY,
