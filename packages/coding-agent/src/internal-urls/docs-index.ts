/**
 * Harness documentation index for the `bbcli://` protocol.
 *
 ...
 * written by `gen:bundle`) — so `@bbcli/pi-coding-agent/*` SDK consumers
 ...
 * SDK consumers importing `@bbcli/pi-coding-agent/*` load TypeScript source, where
...
/** Empty index for when no docs corpus is reachable — degrades `bbcli://` instead of throwing ENOENT at callers. */
function emptyIndex(): DocsIndex {
	logger.warn(
		"bbcli:// docs corpus unavailable: no build-time embed, on-disk docs/ directory, or shipped dist embed found",
	);
