 ...
 * project providers like `.claude/settings.json`, never re-derived from `.bbcli`
...
export interface Provider<T> {
	/** Unique provider ID (e.g., "claude", "bbcli", "mcp-json", "agents-md") */
	id: string;
	 ...
	 * Suggested ranges:
	 *   100+ : Primary providers (bbcli, pi)
...
export interface SourceMeta {
	 ...
	 * `"bbcli"` for bbcli's own registry, `"plugin-dir"` for `--plugin-dir`). Lets
	 * user-scope gating distinguish bbcli's own installs from the foreign Claude
