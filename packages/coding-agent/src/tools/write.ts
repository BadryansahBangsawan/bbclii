...
import type { HighlightStream } from "@bbcli/pi-natives";
import { type Component, Text } from "@bbcli/pi-tui";
import { isEnoent, isRecord, prompt, untilAborted } from "@bbcli/pi-utils";
import {
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			if (resolvedArchivePath) {
				assertTeamWritable(this.session, [resolvedArchivePath.absolutePath]);
			...
			if (resolvedSqlitePath) {
				assertTeamWritable(this.session, [resolvedSqlitePath.absolutePath]);
			...
			if (!internalRouter.canHandle(path)) {
				assertTeamWritable(this.session, [absolutePath]);
			}
