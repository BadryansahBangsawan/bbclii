...
import { $which, getPuppeteerDir, logger, removeWithRetries } from "@bbcli/pi-utils";
import type * as BrowsersNs from "@bbcli/pi-utils/browsers";
...
async function loadBrowsers(): Promise<typeof BrowsersNs> {
	if (!browsersModule) {
		browsersModule = await import("@bbcli/pi-utils/browsers");
	}
