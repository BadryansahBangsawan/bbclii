 ...
 *
 * Instead of every bbcli process launching (and sometimes orphaning) a private
 ...
 * it. The broker stops the daemon when the last bbcli client in the project
 * exits, so Chrome can never outlive bbcli, and concurrent acquisitions across
...
/** Broker-owned browser endpoint one bbcli process can attach to. */
...
export function sharedBrowserDaemonName(headless: boolean): string {
	return headless ? "bbcli.browser.headless" : "bbcli.browser.headed";
}
