 ...
 *
 * Puppeteer clients (the bbcli browser tool: one supervisor connection plus one
 ...
 *   root-event fan-out, so bbcli's own patched-puppeteer client (which
...
class TabState {
	...
	/** True after the relay put this tab in the bbcli group; `ompGroupId` holds that group. */
	grouped = false;
	/** Group RPC in flight — suppresses duplicate requests from load-time tabUpdated bursts. */
	grouping = false;
	ompGroupId: number | undefined;
	/** User pulled the tab out of the bbcli group — never re-group it. */
	groupOptOut = false;
...
export class RelayBridge {
	...
	extClosed(socket: RelaySocket): void {
		...
		for (const tab of this.#tabs.values()) {
			...
			// The extension dissolves bbcli groups on disconnect (or died along
	...
	cdpClosed(connId: number): void {
		...
		conn.sessions.clear();
		// Tabs this client claimed leave the bbcli group unless another claimant
	...
	async #forwardToTab(
		...
		// Relay-private claim: the bbcli tab worker marks the page it was spawned
	...
	async #handleBrowserCommand(conn: CdpConnection, msg: CdpCommand): Promise<void> {
		switch (msg.method) {
			...
			case "Target.createBrowserContext":
				this.#replyError(conn, msg, "Browser contexts are not supported by the bbcli browser relay");
				return;
			default:
	...
	#onTabDetached(tabId: number, reason: string, relayInitiated: boolean): void {
		...
		// down): release the tab's bbcli-group membership too.
		this.#syncTabGrouping(tab);
		this.#retractTab(tab);
	}
	...
	#onTabUpsert(snap: TabSnapshot, opts: { silent?: boolean } = {}): void {
		...
		if (!tab) {
			...
			// The user dragging a tab out of the bbcli group is an opt-out; the
	...
	/** A tab belongs in the bbcli group when claimed by a client, controllable, unpinned, not user-opted-out, and not already in a user group. */
	 ...
	 * duplicate bbcli groups, so at most one group RPC is ever in flight.
	 */
