...
export type RelayRpcRequest =
	...
	/** Add tabs to the per-window bbcli group (created/reused by title), remembering prior membership. */
	| { op: "group"; tabIds: number[]; title: string; color: string }
	/** Return tabs to their pre-bbcli group (or ungroup); no-op for tabs the relay never grouped. */
