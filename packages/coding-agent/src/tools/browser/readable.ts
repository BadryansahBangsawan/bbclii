import type * as DomNs from "@bbcli/pi-utils/dom";
import type * as ReadabilityNs from "@bbcli/pi-utils/readability";
...
async function loadReadability(): Promise<typeof ReadabilityNs> {
	if (!readabilityModule) {
		readabilityModule = await import("@bbcli/pi-utils/readability");
	}
...
async function loadDom(): Promise<typeof DomNs> {
	if (!domModule) {
		domModule = await import("@bbcli/pi-utils/dom");
	}
	return domModule;
}
