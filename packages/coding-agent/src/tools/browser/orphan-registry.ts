 ...
 * Chromium (`bbcli.browser.headless`/`bbcli.browser.headed`).
 *
 * The shared browser outlives any single bbcli process, but tab ownership is
 ...
 * created each shared-browser page target. Any live bbcli process can then reap
...
export interface SharedTargetScope {
	...
	projectDir: string;
	/** Broker daemon name, e.g. `bbcli.browser.headless`. */
	daemonName: string;
}

/** On-disk ownership record: one file per owning bbcli process. */
...
/**
 * Reap shared-browser targets whose owning bbcli process is gone. Each owner
