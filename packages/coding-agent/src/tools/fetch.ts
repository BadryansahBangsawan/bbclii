...
import { type FetchImpl, getEnvApiKey, type ImageContent, type TextContent } from "@bbcli/pi-ai";
import { htmlToMarkdown, notebookToEditableText } from "@bbcli/pi-natives";
import { type Component, Text } from "@bbcli/pi-tui";
import { $which, ptree, truncate } from "@bbcli/pi-utils";
import { type ArchiveFormat, listArchiveRoot, sniffArchiveFormat } from "@bbcli/pi-utils/ar";
...
async function parseFeedToMarkdown(content: string, maxItems = 10): Promise<string> {
	const { parseHTML } = await import("@bbcli/pi-utils/dom");
	try {
