import type { AgentToolContext } from "@bbcli/pi-agent-core";
import type { ImageContent } from "@bbcli/pi-ai";
import { type PtyRunResult, PtySession } from "@bbcli/pi-natives";
import {
	type Component,
...
import type * as XtermModule from "@bbcli/pi-utils/vterm";
import type { Terminal as XtermTerminalType } from "@bbcli/pi-utils/vterm";
...
export async function loadXtermTerminal(): Promise<typeof XtermModule.Terminal> {
	if (!xtermTerminalCtor) {
		const mod = (await import("@bbcli/pi-utils/vterm")) as typeof XtermModule & { default?: typeof XtermModule };
