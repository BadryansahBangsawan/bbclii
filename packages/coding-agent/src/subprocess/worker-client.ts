 ...
 * runtime. The ONNX addons installed on demand under `~/.bbcli/agent/cache/**`
...
function createStderrCapture(exitLabel: string): StderrCapture {
	try {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bbcli-worker-stderr-"));
...
/**
 * Drive the ping/pong readiness probe wired into `bbcli --smoke-test`: send one
