...
- `app.relay: true`: drive the user's Chrome through the bbcli relay. `app.target` selects a tab by URL/title substring; without it, the visible tab is adopted. Opening with `url` navigates that adopted tab.
