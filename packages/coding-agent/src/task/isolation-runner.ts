...
import type * as natives from "@bbcli/pi-natives";
 ...
 *
 * Branch mode: on success, commits the diff onto `bbcli/task/${agentId}` and
	...
	try {
		...
		if (opts.mergeMode === "branch" && result.exitCode === 0) {
			try {
				...
				// commits into the parent ODB and creates `bbcli/task/<id>` before
				...
				const branchName = `bbcli/task/${opts.agentId}`;
