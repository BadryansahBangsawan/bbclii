/**
 * Ownership marker for task-isolation sandboxes under `~/.bbcli/wt/`.
 *
 * Each isolation base dir (`ensureIsolation` in {@link ./worktree}) holds a
 * compact `m` mount plus this marker file naming the bbcli process that created
 * it. `bbcli worktree clear` consults the marker so it can distinguish a live
...
export const ISOLATION_OWNER_FILE = ".bbcli-isolation-owner.json";

/** Recorded owner of a task-isolation sandbox. */
export interface IsolationOwner {
	/** PID of the bbcli process that created and owns the sandbox. */
	pid: number;
 ...
 * `bbcli worktree clear` never sees an owner-less sandbox mid-creation.
 */
...
/**
 * Whether a live bbcli process still owns the sandbox at `baseDir`.
 *
