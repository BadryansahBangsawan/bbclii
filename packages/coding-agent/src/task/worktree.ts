...
import type { VcsCommitAuthor, VcsGitRepo } from "@bbcli/pi-natives";
import * as natives from "@bbcli/pi-natives";
import * as vcs from "@bbcli/pi-natives/vcs";
import { formatBytes, getWorktreeDir, logger, Snowflake } from "@bbcli/pi-utils";
...
async function writeSyntheticTree(
	...
	const tempIndex = path.join(os.tmpdir(), `bbcli-task-index-${Snowflake.next()}`);
...
export async function applyNestedPatches(
	...
	for (const [relativePath, repoPatches] of byRepo) {
		...
		const stashed = (await repository.isDirty())
			? await repository.stashPush(`bbcli-isolation-${Snowflake.next()}`)
			: false;
		try {
...
export async function ensureIsolation(
	...
	for (const candidate of candidates) {
		...
		// `bbcli worktree clear` never sees this sandbox without a live owner,
...
async function replayFilteredAgentCommits(opts: FilteredAgentReplayOptions): Promise<void> {
	...
	const tmpDir = path.join(os.tmpdir(), `bbcli-branch-${Snowflake.next()}`);
	try {
 ...
 * branch named `bbcli/task/${taskId}`. Only root-repo changes go on the branch;
...
export async function commitToBranch(
	...
	const repo = vcs.requireGit(repoRoot);
	const branchName = `bbcli/task/${taskId}`;
...
export async function mergeTaskBranches(
	...
	return withRepoLock(repoRoot, async () => {
		...
		const didStash = await repo.stashPush("bbcli-task-merge");
