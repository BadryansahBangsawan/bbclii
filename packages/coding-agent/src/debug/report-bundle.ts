...
import type { WorkProfile } from "@bbcli/pi-natives";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@bbcli/pi-utils";
import { writeArchive } from "@bbcli/pi-utils/ar";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `bbcli-report-${timestamp}.tar.gz`);
 ...
 * after a crash still captures the fatal PID's `bbcli.<date>.<pid>.log`. Files
