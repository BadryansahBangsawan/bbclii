...
// Reserve room for the `: bbcli-debug-truncated` / `: bbcli-debug-elided` marker
...
// (usage/finish_reason) both stay visible. A `: bbcli-debug-elided` comment
...
function headTailTrim(lines: string[], budget: number, elidedTotal: number): string[] {
	...
	if (i <= j) {
		...
		out.push(`: bbcli-debug-elided chars=${Math.max(0, elided)}`);
		if (tailSlice.length > 0) out.push(`…${tailSlice}`);
	} else if (elided > 0) {
		out.push(`: bbcli-debug-elided chars=${elided}`);
	}
	out.push(...tail);
	return out;
}
...
// Any trimmed result ends with the `: bbcli-debug-truncated` marker carrying
// the original size.
function trimRawLines(raw: string[]): TrimResult {
	...
	lines.push(`: bbcli-debug-truncated originalChars=${originalChars}`);
...
export class RawSseDebugBuffer {
	...
	toRawText(): string {
		...
		const dropped = `: bbcli-debug-dropped records=${this.#droppedRecords} chars=${this.#droppedChars}\n\n`;
