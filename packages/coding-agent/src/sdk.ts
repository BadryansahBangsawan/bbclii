...
import { resolveApiKeyOnce } from "@bbcli/pi-ai/auth-retry";
import type { Dialect } from "@bbcli/pi-ai/dialect";
import {
	getOpenAICodexTransportDetails,
	prewarmOpenAICodexResponses,
} from "@bbcli/pi-ai/providers/openai-codex-responses";
import { FALLBACK_DIALECT, preferredDialect } from "@bbcli/pi-catalog/identity";
...
import { setInvocationConfiguredExtensions, withOmpExtensionRootScope } from "./discovery/bbcli-extension-roots";
...
import type { FileClaimBoard } from "./task/file-claim";
...
export interface CreateAgentSessionOptions {
	...
	/** Global config directory. Default: ~/.bbcli/agent */
	agentDir?: string;
	...
	/** Prompt templates. Default: discovered from cwd/.bbcli/prompts/ + agentDir/prompts/ */
	...
	agentId?: string;
	/** Shared team file-claim board. Same object on Main and every team sibling. */
 ...
 * (`bbcli bench`, dry-balance) build a bare {@link ModelRegistry} that only knows
 ...
 * `~/.bbcli/agent/extensions/`) never reach model resolution. Mirrors the
 * session / `bbcli models` path: drain the queued provider registrations, then
 ...
 *
 * // With explicit model
 * import { getModel } from '@bbcli/pi-ai';
 * const { session } = await createAgentSession({
...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		if (!restrictToolNames) {
			...
			// Discover custom tools from `.bbcli/tools/`, `.claude/tools/`, plugins, etc.
		...
		// cache that `bbcli models find` uses before the online refresh continues in
		// the background.
		...
		if (!model && deferredModelPatterns.length === 0) {
			...
			if (!model) {
				...
				// `bbcli models` (which awaits discovery) lists the model. Await one
		...
		// across bbcli instances by the LSP mux daemon. Session-level because the
