...
import { type AutocompleteProvider, matchesKey, type PasteOptions, type SlashCommand } from "@bbcli/pi-tui";
import { isEnoent, logger, sanitizeText } from "@bbcli/pi-utils";
...
export class InputController {
	...
	handleCtrlZ(): void {
		...
		try {
			...
			// longer stops bbcli: tokio swallows it and the TUI ends up torn down
			...
			//
			// pid=0 (foreground process group, not just our PID): bbcli is not
			// always the shell's direct child. Package-manager launchers (`npx`,
			// `pnpm exec`, `bunx`, …) wait on the real CLI from a parent shim
			// that shares bbcli's process group, and a `bbcli … | tee log` style
	...
	async openExternalEditor(): Promise<void> {
		...
		try {
			this.ctx.ui.stop();
			const result = await openInEditor(editorCmd, currentText, { extension: ".bbcli.md" });
			if (result !== null) {
