...
import { isInsideTmux, wrapTmuxPassthrough } from "@bbcli/pi-tui/terminal-capabilities";
	...
	return {
		emit(event): void {
			...
			const body = {
				...
				// Warp resolves this via CLIAgent.command_prefix(); OhMyPi is "bbcli".
				agent: "bbcli",
	...
	return api => {
		...
		api.on("tool_approval_requested", event => {
			emitter?.emit({
				...
				summary: `bbcli wants to run ${event.toolName}`,
			});
		});
