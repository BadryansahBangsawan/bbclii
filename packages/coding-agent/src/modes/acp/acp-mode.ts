...
import { AgentSideConnection, ndJsonStream, type Stream } from "@bbcli/pi-utils/acp";
...
export async function runAcpMode(createSession: AcpSessionFactory, initialSession?: AgentSession): Promise<void> {
	// Humans who run `bbcli acp` by hand see a silent process and assume it is
	...
	if (process.stdin.isTTY) {
		process.stderr.write(
			"bbcli acp: ACP server speaking JSON-RPC over stdio.\n" +
				'This command is meant to be spawned by an ACP client (e.g. Zed\'s "agent_servers" config), not run directly.\n' +
				"Waiting for protocol frames on stdin; logs: ~/.bbcli/logs/\n",
		);
	}
