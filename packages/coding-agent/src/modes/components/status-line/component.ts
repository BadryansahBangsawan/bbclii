...
import type { AssistantMessage, UsageLimit, UsageReport } from "@bbcli/pi-ai";
import {
...
import { adjustHsv, formatNumber, getProjectDir, hexToRgb, rgbToHex } from "@bbcli/pi-utils";
...
/** Brand-color fade duration across working-state edges (rust bbcli's `BRAND_FADE`). */
const BRAND_FADE_MS = 450;
/** Repaint cadence while the brand fade is in flight (rust bbcli's `FADE_FRAME`). */
...
export class StatusLineComponent implements Component {
	 ...
	 * turn runs — a port of rust bbcli's status-band brand fade (450ms cubic
	...
	#sampleBrandHex(settledHex: string, now: number): string {
		...
		// Cubic ease-in-out, matching rust bbcli's Easing::EaseInOut.
	...
	#buildStatusLine(
		...
		// (rust bbcli's status band). Like the other caps it needs an opaque
