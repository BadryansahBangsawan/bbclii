...
import { getTimeBasedPricingPeriod } from "@bbcli/pi-catalog/models";
import { SPINNER_ADVANCE_MS, TERMINAL } from "@bbcli/pi-tui";
import { formatDuration, formatNumber, getProjectDir, pathIsWithin, relativePathWithinRoot } from "@bbcli/pi-utils";
...
const piSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		// whole-unit turn timer (port of rust bbcli's status-band active brand).
...
/** Turn timer in bbcli's brand format: whole seconds → minutes → hours (capped at 99h). */
...
const modeSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		if (plan && (plan.enabled || plan.paused)) {
			const label = plan.paused ? `Plan${pauseSuffix}` : plan.ultraplan ? "Ultraplan" : "Plan";
