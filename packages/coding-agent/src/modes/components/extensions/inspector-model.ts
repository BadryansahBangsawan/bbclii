...
import { arkToWireSchema, isArkSchema } from "@bbcli/pi-ai/utils/schema";
import { normalizePathForComparison, parseFrontmatter } from "@bbcli/pi-utils";
...
/** Project-local items only. Uses the directory that contains `.bbcli`, when present. */
export function projectListHint(ext: Extension): string | undefined {
	...
	const ompIndex = parts.lastIndexOf(".bbcli");
