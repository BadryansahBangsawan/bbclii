...
export class PluginListComponent extends OverlayPanel {
	...
	constructor(
		...
		if (entries.length === 0) {
			...
			this.addChild(new Spacer(1));
			this.addChild(new Text(theme.fg("dim", "Install npm plugins:        bbcli plugin install <package>"), 0, 0));
			this.addChild(
				new Text(theme.fg("dim", "Install marketplace plugins: bbcli plugin install <name>@<marketplace>"), 0, 0),
			);
			this.addChild(new Spacer(1));
