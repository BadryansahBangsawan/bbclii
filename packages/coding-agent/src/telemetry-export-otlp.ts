...
/**
 * Periodic flush interval. A long-lived `bbcli` process (the ACP server is
...
export async function registerProviders(signalConfig: TelemetrySignalConfig): Promise<void> {
	...
	if (signalConfig.metric) {
		...
		metricRecorder = new AgentMetricRecorder(metrics.getMeter("@bbcli/pi-coding-agent"));
	}
	...
	if (signalConfig.log) {
		...
		otelLogger = logProvider.getLogger("@bbcli/pi-coding-agent");
