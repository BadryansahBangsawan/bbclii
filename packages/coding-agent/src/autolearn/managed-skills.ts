 ...
 * isolated directory (`~/.bbcli/agent/managed-skills`) separate from
 * user-authored skills (`~/.bbcli/agent/skills`). They are discovered and
...
import { getAgentDir, isEnoent } from "@bbcli/pi-utils";
import { YAML } from "bun";

/** Provider id stamped on discovered managed skills (distinguishes them from authored). */
export const MANAGED_SKILLS_PROVIDER_ID = "bbcli-managed";
...
/** Resolve the isolated managed-skills directory (`~/.bbcli/agent/managed-skills`). */
