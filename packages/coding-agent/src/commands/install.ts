/**
 * `bbcli install <target>` — top-level convenience over `bbcli plugin install` /
 * `bbcli plugin link`.
 *
 * The docs (omp.sh/docs/extension-authoring) advertise
 *
 *   bbcli install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `bbcli install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: bbcli install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
