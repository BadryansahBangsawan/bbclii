...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.bbcli/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.bbcli/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  bbcli agents unpack",
		"# Export bundled agents into project config\n  bbcli agents unpack --project",
		"# Overwrite existing local agent files\n  bbcli agents unpack --project --force",
		"# Export into a custom directory\n  bbcli agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("bbcli", "agents", Agents);
			return;
		}
