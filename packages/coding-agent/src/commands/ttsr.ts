...
/**
 * `bbcli ttsr` — inspect and test Time-Traveling Stream Rules.
 *
 * `bbcli ttsr test` feeds a snippet (inline, --file, or stdin) through the real
 * TTSR matching pipeline and reports which rules would trigger. `bbcli ttsr list`
...
export default class Ttsr extends Command {
	...
	static examples = [
		"bbcli ttsr list",
		"bbcli ttsr test 'const x: any = 1'",
		"bbcli ttsr test src/foo.ts",
		"bbcli ttsr test --file src/foo.ts",
		"bbcli ttsr test --file src/foo.ts --source text",
		"bbcli ttsr test --rule .bbcli/rules/no-any.md --source tool --path src/foo.ts 'const x: any = 1'",
		"bbcli ttsr test --agent scout 'const x: any = 1'",
		"echo 'Box::leak(&mut v)' | bbcli ttsr test --file - --path src/lib.rs",
		"bbcli ttsr test --source tool --tool edit --path src/foo.ts 'const x: any = 1'",
		"bbcli ttsr scan",
		"bbcli ttsr scan src/",
		"bbcli ttsr scan -r .bbcli/rules/no-any.md src/",
	];
	...
	async run(): Promise<void> {
		...
		// inline text — so `bbcli ttsr test src/foo.ts` works without --file.
