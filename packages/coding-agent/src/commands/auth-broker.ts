/**
 * `bbcli auth-broker` — manage the bbcli credential vault.
 */

import { Args, Command, Flags, renderCommandHelp } from "@bbcli/pi-utils/cli";
import {
	AUTH_BROKER_ACTIONS,
...
export default class AuthBroker extends Command {
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  bbcli auth-broker serve",
		"# Boot on a non-default port\n  bbcli auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  bbcli auth-broker token",
		"# Rotate the bearer token\n  bbcli auth-broker token --regenerate",
		"# List supported OAuth providers\n  bbcli auth-broker list",
		"# Local login (run on the broker host)\n  bbcli auth-broker login anthropic",
		"# Interactive provider selection\n  bbcli auth-broker login",
		"# Remote login over SSH tunnel\n  bbcli auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  bbcli auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  bbcli auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  bbcli auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  bbcli auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  bbcli auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  bbcli auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("bbcli", "auth-broker", AuthBroker);
			return;
		}
