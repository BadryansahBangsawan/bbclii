/**
 * `bbcli browser-relay` — drive the user's own Chrome tabs.
 */
...
export default class BrowserRelay extends Command {
	...
	static flags = {
		...
		dir: Flags.string({
			description: "Extension install directory (install; default ~/.bbcli/browser-relay/extension)",
		}),
		"no-group": Flags.boolean({
			description: "Don't gather controllable tabs into an 'bbcli' tab group",
			default: false,
		}),
	...
	static examples = [
		"bbcli browser-relay install    # write the Chrome extension to disk + setup steps",
		"bbcli browser-relay            # serve the relay on the default port",
		"bbcli browser-relay -p 9333 --token s3cret",
	];
