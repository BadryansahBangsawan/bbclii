 ...
 */
import type { CommandEntry } from "@bbcli/pi-utils/cli";
...
// commands. Without a guard `resolveCliArgv` rewrites e.g. `bbcli marketplace add
// xyz` to `bbcli launch marketplace add xyz`, silently forwarding the argv to the
...
// The real commands live under `bbcli plugin <action>`; each entry maps a verb to
...
const RESERVED_TOP_LEVEL_WORDS: Record<string, string> = {
	extensions:
		'`bbcli extensions` is not a management command. Use `bbcli plugin list` / `bbcli plugin install`, or run `bbcli launch extensions` if you meant to send "extensions" as a prompt.',
	list: '`bbcli list` is not a top-level command. Use `bbcli plugin list` to list installed plugins, or run `bbcli launch list` if you meant to send "list" as a prompt.',
	remove:
		'`bbcli remove` is not a top-level command. Use `bbcli plugin uninstall <name>` to remove a plugin, or run `bbcli launch remove` if you meant to send "remove" as a prompt.',
	uninstall:
		'`bbcli uninstall` is not a top-level command. Use `bbcli plugin uninstall <name@marketplace>` to remove a plugin, or run `bbcli launch uninstall` if you meant to send "uninstall" as a prompt.',
	marketplace:
		'`bbcli marketplace` is not a top-level command. Use `bbcli plugin marketplace <add|remove|update|list>` to manage marketplaces, or run `bbcli launch marketplace` if you meant to send "marketplace" as a prompt.',
	discover:
		'`bbcli discover` is not a top-level command. Use `bbcli plugin discover [marketplace]` to browse available plugins, or run `bbcli launch discover` if you meant to send "discover" as a prompt.',
	upgrade:
		'`bbcli upgrade` is not a top-level command. Use `bbcli plugin upgrade [name@marketplace]` to upgrade plugins, or run `bbcli launch upgrade` if you meant to send "upgrade" as a prompt.',
	enable:
		'`bbcli enable` is not a top-level command. Use `bbcli plugin enable <name@marketplace>` to enable a plugin, or run `bbcli launch enable` if you meant to send "enable" as a prompt.',
	disable:
		'`bbcli disable` is not a top-level command. Use `bbcli plugin disable <name@marketplace>` to disable a plugin, or run `bbcli launch disable` if you meant to send "disable" as a prompt.',
};

// Sub-actions that make `bbcli marketplace <sub>` unambiguously a management
// command even when multi-word (the reporter's `bbcli marketplace add xyz`,
 ...
 *
 * A bare verb (`bbcli marketplace`) always hints. A multi-word invocation only
 * hints when the arguments follow the documented plugin grammar — a marketplace
 * sub-action (`bbcli marketplace add …`) or a `name@marketplace` plugin id
 * (`bbcli uninstall foo@bar`) — so genuine prompts that merely begin with one of
 * these words (`bbcli list all my files`, `bbcli upgrade the deps`) still launch.
 *
...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	// (`bbcli --approval-mode=yolo acp`). `run` dispatches strictly on argv[0], so
	...
	// it (`bbcli --cwd <dir> update`) are stripped rather than forwarded into a
