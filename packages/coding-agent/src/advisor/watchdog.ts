...
import { getAgentDir, isEnoent, logger, prompt } from "@bbcli/pi-utils";
 ...
 * `.bbcli/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.bbcli/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .bbcli/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
			candidates.add(path.resolve(current, ".bbcli", filename));
	...
	for (const candidate of candidates) {
		try {
			...
			const ownerDir = baseName === ".bbcli" ? path.dirname(parent) : parent;
			const ownerBaseName = ownerDir.split(path.sep).pop() ?? "";
			if (isUser || !ownerBaseName.startsWith(".") || baseName === ".bbcli") {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project .bbcli folder, and user agent dir.
