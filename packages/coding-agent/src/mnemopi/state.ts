...
import type * as MnemopiNs from "@bbcli/pi-mnemopi";
import type { Mnemopi, RecallResult } from "@bbcli/pi-mnemopi";
import type * as MnemopiCoreNs from "@bbcli/pi-mnemopi/core";
import type { LocalModelInitializer } from "@bbcli/pi-mnemopi/core";
import { logger, toError } from "@bbcli/pi-utils";
import {
	composeRecallQuery,
	formatCurrentTime,
...
/**
 * Lazily load `@bbcli/pi-mnemopi` (memoized) and route fastembed loads
...
export async function loadMnemopi(): Promise<typeof MnemopiNs> {
	if (!mnemopiMod) {
		mnemopiMod = await import("@bbcli/pi-mnemopi");
		installLocalModelInitializer(mnemopiMod.setLocalModelInitializer);
	}
	return mnemopiMod;
}

/** Lazily load `@bbcli/pi-mnemopi/core` (memoized). */
export async function loadMnemopiCore(): Promise<typeof MnemopiCoreNs> {
	if (!mnemopiCoreMod) {
		mnemopiCoreMod = await import("@bbcli/pi-mnemopi/core");
