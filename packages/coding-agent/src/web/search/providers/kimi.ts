 ...
 * Kimi Code Console credential (`bbcli /login kimi-code` or an explicit
 * `MOONSHOT_SEARCH_API_KEY` / `KIMI_SEARCH_API_KEY`), not `MOONSHOT_API_KEY`.
 * Endpoint: POST https://api.kimi.com/coding/v1/search
 */
import { type ApiKey, type AuthStorage, type FetchImpl, withAuth } from "@bbcli/pi-ai";
 ...
 * stored `kimi-code` credential (from `bbcli /login kimi-code`), so a stale token
...
export async function searchKimi(params: KimiSearchParams): Promise<SearchResponse> {
	...
	if (!keyOrResolver) {
		throw new Error(
			"Kimi search credentials not found. Kimi web search uses the Kimi Code service (api.kimi.com); set MOONSHOT_SEARCH_API_KEY / KIMI_SEARCH_API_KEY to a Kimi Code Console key, or login with 'bbcli /login kimi-code'. A Moonshot Open Platform key (MOONSHOT_API_KEY) is not accepted here.",
		);
	}
