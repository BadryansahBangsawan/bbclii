...
import { resolveCodexResponsesUrl } from "@bbcli/pi-ai/providers/openai-codex-responses";
...
import { $env, readSseJson, USER_AGENT } from "@bbcli/pi-utils";
...
export async function searchCodex(params: SearchParams): Promise<SearchResponse> {
	...
	if (transport.customEndpoint) {
		...
		if (!seed) {
			throw new Error(
				"No Codex OAuth credentials found. Login with 'bbcli /login openai-codex' to enable Codex web search.",
			);
		}
