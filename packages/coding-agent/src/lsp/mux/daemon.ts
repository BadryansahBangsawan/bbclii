 ...
 * first use, stopped when the last bbcli process in the project exits), dials
 * its socket, performs the `bbcli/muxConnect` handshake, and returns an
...
import { logger, ptree } from "@bbcli/pi-utils";
...
export async function smokeTestLspMux(): Promise<void> {
	const endpoint =
		process.platform === "win32"
			? `\\\\.\\pipe\\bbcli-lsp-mux-smoke-${process.pid.toString(16)}`
			: path.join(os.tmpdir(), `bbcli-lsp-mux-smoke-${process.pid.toString(36)}.sock`);
	const spawn = resolveWorkerSpawnCmd(LSP_MUX_WORKER_ARG);
