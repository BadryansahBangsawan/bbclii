...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@bbcli/pi-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.bbcli/, .pi/, .claude/)
	...
	// User config directories (~/.bbcli/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .bbcli/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.bbcli/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
