...
import { isEnoent, logger, postmortem } from "@bbcli/pi-utils";
 ...
 * deleting a scope whose owning bbcli process is mid-startup (token written, broker
...
/** Handle keeping one bbcli process registered in a project daemon scope. */
export interface DaemonProjectPresence {
	close(): Promise<void>;
}

/** Register this bbcli process so project daemons survive while it remains alive. */
...
/** Return whether a registered bbcli process in this runtime directory is still alive. */
