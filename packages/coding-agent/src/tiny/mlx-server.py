...
# serves every bbcli process on the machine; exits on its own once idle.
#
...
COMPLETE_MARKER = ".bbcli-complete.json"
...
def _request(url):
    headers = {"User-Agent": "bbcli-tiny-mlx"}
    if HF_TOKEN:
...
class Server:
    ...
    def serve(self, socket_path):
        ...
        sys.stdout.write(f"bbcli tiny worker listening on {socket_path}\n")
        sys.stdout.flush()
        while True:
