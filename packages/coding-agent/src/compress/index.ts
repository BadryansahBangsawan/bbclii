/**
 * `bbcli compress` — rewrite text files into the dense prompt register.
 *
...
import { getProjectDir, prompt, sanitizeText } from "@bbcli/pi-utils";
...
/** User-facing options for `bbcli compress`. */
...
export async function resolveCompressTargets(patterns: readonly string[], cwd: string): Promise<string[]> {
	...
	for (const pattern of patterns) {
		if (/[*?[\]{}]/.test(pattern)) {
			// `dot: true` — prompt corpora live under dot directories such as `.bbcli/commands`.
 ...
 * stdout for a single-file run (`bbcli compress f.md > out.md`).
 */
