...
import { getSafeProjectCwd, logger, postmortem } from "@bbcli/pi-utils";
...
echo "# Shell snapshot - generated by bbcli agent" >| "$SNAPSHOT_FILE"
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), uid === undefined ? "bbcli-shell-snapshots" : `bbcli-shell-snapshots-${uid}`);
