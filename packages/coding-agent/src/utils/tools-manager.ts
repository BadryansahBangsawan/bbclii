...
import { $which, getToolsDir, logger, ptree, TempDir, USER_AGENT } from "@bbcli/pi-utils";
import { extractArchive } from "@bbcli/pi-utils/ar";
...
async function downloadTool(tool: ToolName, signal?: AbortSignal): Promise<string> {
	...
	// Extract
	const tmp = await TempDir.create("@bbcli-tools-extract-");
