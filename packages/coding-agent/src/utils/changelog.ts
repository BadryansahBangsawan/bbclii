...
export function formatStartupChangelogSummary(selection: StartupChangelogSelection): string {
	...
	if (!latestVersion || selection.selectedEntries === 0) {
		return "Updated bbcli. Use /changelog for recent changes.";
	}
...
/**
 * Parse changelog entries from bbcli's package asset when available, falling back
...
/**
 * Parse an bbcli changelog marker version into comparable parts.
 */
...
/**
 * Last bbcli version whose changelog the user has seen. Stored as a plain-text
 * marker file (`~/.bbcli/agent/last-changelog-version`) rather than in
