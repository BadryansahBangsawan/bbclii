...
import { $which, getSafeProjectCwd, logger } from "@bbcli/pi-utils";
...
const HEALTH_PATH = "/.well-known/bbcli-blob-health";
...
async function spawnUrlTunnel(
	...
	const logPath = path.join(os.tmpdir(), `bbcli-blob-tunnel-${Date.now().toString(36)}-${process.pid}.log`);
