 ...
 *
 * Handles `bbcli read` — invokes the `read` agent tool against a path/URL and
	...
	try {
		if (shouldDiscoverMcp(cmd.path)) {
			...
			const result = await discoverAndLoadMCPTools(cwd, {
				...
				filterExa: true,
				// `bbcli read` has no Eval prelude, so browser MCP remains available.
				filterBrowser: false,
		...
		const result = await tool.execute("bbcli-read", { path: cmd.path });
