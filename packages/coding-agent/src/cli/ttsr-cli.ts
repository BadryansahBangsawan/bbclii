 ...
 *
 * `bbcli ttsr test` — feed a snippet (inline text, `--file`, or stdin) through the
 ...
 *
 * `bbcli ttsr list` — show every TTSR-registered rule the current project/user
 * config would load, with its conditions, scope, and source.
 *
 * `--agent <name>` on `bbcli ttsr test` evaluates the rule's `agents` frontmatter
 * scoping as that agent (default "main"); `bbcli ttsr list`/`scan` stay unfiltered.
 */
import * as fs from "node:fs";
import * as path from "node:path";
import { AstMatchStrictness, astMatch, FileType, type GlobMatch, glob } from "@bbcli/pi-natives";
import chalk from "@bbcli/pi-utils/chalk";
import { getProjectDir } from "@bbcli/pi-utils/dirs";
import {
...
async function runTest(args: TtsrTestArgs, json: boolean, cwd: string): Promise<void> {
	...
	if (rules.length === 0) {
		const msg = args.rule
			...
			: `No TTSR rules registered for this project as agent "${agent}". Rules scoped to other agents via \`agents\` are excluded — run \`bbcli ttsr list\` to see every rule, or pass --agent <name>.`;
		if (json) {
