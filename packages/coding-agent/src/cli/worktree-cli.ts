/**
 * CLI handler for `bbcli worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.bbcli/wt/`:
 *
 ...
 *     marker naming the live bbcli process; a
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.bbcli/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `bbcli worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.bbcli/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
