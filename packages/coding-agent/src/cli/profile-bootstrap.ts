 ...
 * `@bbcli/pi-utils/env`, which eagerly loads `.env` from the agent directory
 ...
 * doesn't look like another flag. Without this, `bbcli --system-prompt --profile
 ...
 * `bbcli --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`bbcli --print --profile work`).
 */
 ...
 * positionals are never stolen (`bbcli grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `bbcli launch --profile work` and
 * `bbcli acp --profile work` still select profile `work`.
 *
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `bbcli --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`bbcli --some-ext-bool --profile work`
		...
		// activates (`bbcli --print --profile work`).
