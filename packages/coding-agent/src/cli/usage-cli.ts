 ...
 *
 * Handles `bbcli usage` — fetches provider usage reports for every
...
import { AuthBrokerClient } from "@bbcli/pi-ai/auth-broker";
import type { ClientUsageClientSummary } from "@bbcli/pi-ai/usage";
import { formatDuration, formatNumber, sanitizeText } from "@bbcli/pi-utils";
...
/**
 * Tombstones worth a row in `bbcli usage`: OAuth credentials torn down
 ...
 * `bbcli usage --provider xai` can still confirm the stored credential has no
 * usage endpoint.
 */
	...
	try {
		...
		if (cmd.history) {
			...
			if (entries.length === 0) {
				...
				process.stderr.write(
					chalk.yellow(
						`No usage history recorded${scope} yet. Snapshots accumulate whenever usage is fetched (TUI footer, /usage, bbcli usage).\n`,
					),
				);
				process.exitCode = 1;
				return;
		...
		if (filteredReports.length === 0 && accounts.length === 0) {
			...
			const message =
				storedAccounts.length > 0
					...
					: `No credentials found${scope}. Run \`bbcli\` and use /login to add accounts.\n`;
