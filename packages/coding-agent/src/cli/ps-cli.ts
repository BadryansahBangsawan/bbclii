/**
 * CLI handler for `bbcli ps` — inspect and control processes supervised by the
 * daemon broker from outside the harness.
 *
 * A bare `bbcli ps` on a TTY opens the interactive alt-screen monitor
...
export async function runPsCommand(cmd: PsCommandArgs): Promise<void> {
	try {
		...
		if (!cmd.name) {
			console.error(chalk.red(`${cmd.action} requires a process name. Run \`bbcli ps\` to list processes.`));
			process.exitCode = 1;
			return;
		}
