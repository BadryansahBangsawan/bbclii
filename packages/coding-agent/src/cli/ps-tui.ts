/**
 * Interactive alt-screen monitor for `bbcli ps` (btop idiom): a live process
 ...
 */
import { type Component, matchesKey, ProcessTerminal, TUI, truncateToWidth } from "@bbcli/pi-tui";
import { formatDuration } from "@bbcli/pi-utils";
...
class PsTopComponent implements Component {
	...
	#header(width: number, title: string): string {
		...
		const left = ` ${chalk.bold("bbcli ps")} ${chalk.dim("·")} ${title}`;
