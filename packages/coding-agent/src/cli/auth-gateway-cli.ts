/**
 * `bbcli auth-gateway` command handlers.
 *
...
} from "@bbcli/pi-ai/auth-broker";
import { DEFAULT_AUTH_GATEWAY_BIND, startAuthGateway } from "@bbcli/pi-ai/auth-gateway";
import { type GeneratedProvider, getBundledModels } from "@bbcli/pi-catalog/models";
import { getConfigRootDir, isEnoent, logger, VERSION } from "@bbcli/pi-utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`bbcli auth-gateway serve` requires OMP_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). The gateway is itself a broker client.",
		);
	}
	...
	// cached + broker-discovered), so the discovery-only models bbcli itself
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	try {
		...
		if (flags.json) {
			...
			if (!tokenPresent) {
				process.stdout.write(
					"Run `bbcli auth-gateway token` or `bbcli auth-gateway serve` to create a bearer token.\n",
				);
			}
		}
...
/**
 * `bbcli auth-gateway check` — probe each broker-supplied credential and print
...
async function runCheck(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`bbcli auth-gateway check` requires OMP_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). It probes the same credentials the gateway would serve.",
		);
	}
