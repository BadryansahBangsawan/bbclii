/**
 * `bbcli models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.bbcli/models.db`
 ...
 */
import type { Api, Effort, Model } from "@bbcli/pi-ai";
 ...
 * name doubles as an `bbcli models <provider>` shortcut.
 */
...
/** `bbcli models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `bbcli models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`bbcli models find` requires a search substring, e.g. `bbcli models find minimax`\n");
		process.exitCode = 1;
		return;
	}
