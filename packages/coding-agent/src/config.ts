...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@bbcli/pi-utils";
 ...
 * project with bbcli's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as bbcli's startup changelog).
 */
...
/**
 * Path to bbcli's own `CHANGELOG.md`, or `undefined` when the package directory
...
/**
 * User-level: ~/.bbcli/agent, Claude's active config directory, ~/.codex, ~/.gemini
 * Project-level: .bbcli, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".bbcli", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.bbcli/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.bbcli/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.bbcli/agent/commands", source: ".bbcli", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.bbcli, .claude) - the nearest one found.
