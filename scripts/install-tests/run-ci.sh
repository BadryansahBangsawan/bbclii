...
mkdir -p "$BINARY_DIR"
cp packages/coding-agent/dist/bbcli "$BINARY_DIR/bbcli"
smoke_cli "$BINARY_DIR/bbcli"
...
(
   ...
   smoke_cli "$BUN_INSTALL/bin/bbcli"
)
...
stats_tgz="$(find_tarball "$TARBALL_DIR"/oh-my-pi-bbcli-stats-*.tgz)"
...
(
   ...
   smoke_cli ./node_modules/.bin/bbcli
)
