...
describe("published legal payloads", () => {
	...
	it("stages missing legal files without replacing package-local text", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-publish-legal-"));
	...
	it("lists every legal file explicitly in the native core package", async () => {
		const pkgDir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-native-core-"));
		try {
			await Bun.write(
				path.join(pkgDir, "package.json"),
				JSON.stringify({
					name: "@bbcli/pi-natives",
					version: "15.5.15",
					license: "MIT",
				}),
			);
