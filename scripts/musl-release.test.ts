...
describe("musl release artifacts", () => {
	test("builds the requested x64 and arm64 musl asset names with Bun's musl targets", async () => {
		...
		expect(result.stdout).toContain(
			"Bun.build target=bun-linux-x64-musl-baseline outfile=packages/coding-agent/binaries/bbcli-linux-musl-x64",
		);
		expect(result.stdout).toContain(
			"Bun.build target=bun-linux-arm64-musl outfile=packages/coding-agent/binaries/bbcli-linux-musl-arm64",
		);
	});

	test("selects the musl asset when the Linux host reports musl", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bbcli-musl-install-"));
		tempDirs.push(dir);
...
case "$*" in
  ...
  *) while [ "$#" -gt 0 ]; do
       [ "$1" = "-o" ] && { printf '%s\n' '#!/bin/sh' 'echo "bbcli v1.0.0"' > "$2"; exit 0; }
       shift
     done ;;
esac
`,
		...
		expect(result.stdout).toContain("Downloading bbcli-linux-musl-x64...");
		expect(await Bun.file(path.join(installDir, "bbcli")).text()).toBe('#!/bin/sh\necho "bbcli v1.0.0"\n');
	});
});
