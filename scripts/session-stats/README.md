...
(`~/.bbcli/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.bbcli/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.bbcli/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@bbcli/pi-ai`,
credentials resolved through bbcli's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.bbcli/stats-audit-cache.json` (keyed by session id +
