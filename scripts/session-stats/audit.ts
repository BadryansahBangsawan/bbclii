#!/usr/bin/env bun
/**
 * Token-usage audit over the local bbcli session corpus (~/.bbcli/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @bbcli/pi-ai)
 * to judge:
 ...
 *
 * Auth: resolves an API key for the classifier provider through bbcli's auth
 * storage (~/.bbcli/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { type GeneratedProvider, getBundledModel } from "@bbcli/pi-catalog/models";
import { getAgentDbPath, isEnoent } from "@bbcli/pi-utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".bbcli", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".bbcli", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.bbcli/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.bbcli/stats-audit-cache.json)\n` +
...
async function openClassifier(modelSpec: string): Promise<Classifier> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (bbcli login or env var required)`);
	}
	return { model, apiKey };
}
