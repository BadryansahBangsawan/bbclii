...
def resolve_omp_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `bbcli` on PATH. Set --bbcli-bin or OMP_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through bbcli RPC mode."
    )
    parser.add_argument("--bbcli-bin", default=os.environ.get("OMP_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"bbcli-fixture-runs-{timestamp}"
    )
